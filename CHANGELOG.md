...
- **Electrum script-hash subscription transport — CRD ch.38 R38.6.5 (partial).** Groundwork for push-driven balance updates on Electrum-backed coins. The client already received server-initiated notifications but recognised exactly one method and discarded everything else; it now routes `blockchain.scripthash.subscribe` notifications to a registry keyed by script hash, exposes `subscribe_scripthash` / `resubscribe_watched_scripthashes`, remembers every watched hash, and re-arms them on each newly connected server — a subscription belongs to one session, so a reconnect or server swap drops it silently and nothing may assume otherwise. Unrecognised notification methods are now ignored rather than logged as errors, since a server may push subscriptions we never requested. **Not yet user-visible:** no caller subscribes and nothing consumes the wake signal, so balance updates remain purely poll-driven at their existing interval. The consumer wiring is stage 4 of `docs/plans/faster-transaction-visibility.md`. Polling is retained as the correctness floor by design, so a lost or absent subscription can only cost latency, never freshness. Code: `mm2src/coins/utxo/rpc_clients/electrum_rpc_client.rs`, `mm2src/coins/utxo/utxo_builder/utxo_coin_builder.rs`.
