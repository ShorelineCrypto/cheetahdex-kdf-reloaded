...
impl HttpStatusCode for OrderbookRpcError {
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            OrderbookRpcError::InternalError(_) | OrderbookRpcError::P2PSubscribeError(_) => {
...
pub async fn orderbook_rpc_v2(
    ...
    let base_coin_conf = get_tradeable_coin_conf(&ctx, &req.base).mm_err(Into::into)?;

    let rel_coin_conf = get_tradeable_coin_conf(&ctx, &req.rel).mm_err(Into::into)?;
    ...
    let orderbook = ordermatch_ctx.orderbook.lock();
    let my_pubsecp = mm2_internal_pubkey_hex(&ctx).map_to_mm(OrderbookRpcError::InternalError)?;
