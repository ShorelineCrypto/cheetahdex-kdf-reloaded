...
// ─── ToBytes adapters for V2 swap associated types ──────────────────────────
// `keys::Public` and `keys::Signature` are covered by the blanket
// `impl<T: AsRef<[u8]>> ToBytes for T` thanks to their `AsRef<[u8]>` impls
// in `kdf_keys`. The `Preimage` associated type is a local newtype
// (`utxo_standard_swap_v2::UtxoTxPreimage`) with its own `ToBytes` impl,
// because coherence forbids implementing `ToBytes` directly for the
// foreign `script::TransactionInputSigner`.
