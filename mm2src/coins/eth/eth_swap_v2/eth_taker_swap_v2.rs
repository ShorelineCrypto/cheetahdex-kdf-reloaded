...
impl EthCoin {
    ...
    pub(crate) async fn validate_taker_funding_impl(
        ...
        match self.coin_type {
            EthCoinType::Eth => {
                ...
                validate_eth_taker_payment_data(&decoded, &validation_args, function, tx.value)?;
            },
    ...
    pub(crate) async fn find_taker_payment_spend_tx_impl(
        ...
        let taker_swap_v2_contract = self
            ...
            .taker_swap_v2_contract;
        let id_array = extract_id_from_tx_data(&taker_payment.data, &TAKER_SWAP_V2, TAKER_PAYMENT_APPROVE)
            .await?
            .as_slice()
            .try_into()?;
    ...
    /// Constructs the encoded transaction input data for `takerPaymentApprove`.
    ...
    /// Prepares data for EtomicSwapTakerV2 contract spendTakerPayment method
    ...
    /// Extracts the maker's secret from the input of transaction that calls `spendTakerPayment`.
    pub(crate) async fn extract_secret_v2_impl(&self, spend_tx: &SignedEthTx) -> Result<[u8; 32], String> {
        ...
        let signature = &spend_tx.data[0..4];
    ...
    /// Retrieves the payment status from a given smart contract address.
...
fn validate_eth_taker_payment_data(
    ...
    for (index, expected_token, field_name) in checks {
        let token = get_function_input_data(decoded, func, index).map_to_mm(ValidateSwapV2TxError::InternalError)?;
