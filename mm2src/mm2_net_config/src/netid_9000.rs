//! Network configuration for netid 9000 — regtest / docker-test only.
//!
//! Compiled in only when the `regtest-netid` feature is enabled. The
//! production `mm2` binary never enables this feature, so netid 9000
//! is unreachable in shipped builds.
//!
//! All fee parameters mirror netid 8762 (AtomicDEX) so the regtest
//! harness exercises the same code paths as the default network. The
//! DEX fee receivers are intentionally the netid-8762 ones — regtest
//! coins have no real value, so the addresses are inert.
...
lazy_static! {
    static ref DEX_FEE_ADDR_RAW: Vec<u8> =
        hex::decode(DEX_FEE_ADDR_PUBKEY).expect("netid_9000: invalid DEX_FEE_ADDR_PUBKEY hex");
}

pub struct Netid9000;

impl NetConfig for Netid9000 {
    fn netid(&self) -> u16 { 9000 }
