...
impl<'a> SqlQuery<'a> {
        ...
        F: FnMut(&Row<'_>) -> SqlResult<B>,
    {
        ...
        let params_ref: Vec<&dyn ToSql> = self.params().iter().map(|p| p as &dyn ToSql).collect();
        let mut stmt = self.conn.prepare(&sql)?;
        let items = stmt
            .query_map(params_ref.as_slice(), f)?
        ...
        F: FnOnce(&Row<'_>) -> SqlResult<T>,
    {
        ...
        debug!("Trying to execute SQL query {} with params {:?}", sql, self.params());
        let params_ref: Vec<&dyn ToSql> = self.params().iter().map(|p| p as &dyn ToSql).collect();
        query_single_row(self.conn, &sql, params_ref.as_slice(), f)
    }
