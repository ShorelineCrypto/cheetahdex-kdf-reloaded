...
- **WalletConnect** (`mm2src/kdf_walletconnect/`) is a post-anchor feature whose integration code is **lineage-derived from the upstream/GLEEC KDF WalletConnect work product** (same crate path; the lineage introduced it earlier). It is carried under GPLv2 copyleft and is **not** a clean-room original. The substantive files are classified accordingly in the provenance ledger.
