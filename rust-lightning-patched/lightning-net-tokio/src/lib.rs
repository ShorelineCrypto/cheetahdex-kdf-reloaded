...
impl Connection {
    async fn schedule_read<CMH, RMH, L, UMH>(
        peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor, Arc<CMH>, Arc<RMH>, Arc<L>, Arc<UMH>>>,
        us: Arc<Mutex<Self>>,
    ...
    UMH: CustomMessageHandler + 'static + Send + Sync,
{
    ...
    let handle_opt = if let Ok(_) = peer_manager.new_inbound_connection(
    ...
    UMH: CustomMessageHandler + 'static + Send + Sync,
{
    if let Ok(Ok(stream)) = time::timeout(Duration::from_secs(10), async {
        TcpStream::connect(&addr).await.map(|s| s.into_std().unwrap())
    })
    .await
    {
...
mod tests {
    ...
    async fn do_basic_connection_test() {
        ...
        let a_manager = Arc::new(PeerManager::new(
            ...
            &[1; 32],
            Arc::new(TestLogger()),
            Arc::new(lightning::ln::peer_handler::IgnoringMessageHandler {}),
        ));
        ...
        let (conn_a, conn_b) = if let Ok(listener) = std::net::TcpListener::bind("127.0.0.1:9735") {
            (
                std::net::TcpStream::connect("127.0.0.1:9735").unwrap(),
            ...
            (
                std::net::TcpStream::connect("127.0.0.1:46926").unwrap(),
                listener.accept().unwrap().0,
            )
        } else {
