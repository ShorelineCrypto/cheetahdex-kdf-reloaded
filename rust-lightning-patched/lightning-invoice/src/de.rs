...
use super::{
    constants, Currency, Description, ExpiryTime, Fallback, Invoice, InvoiceFeatures, InvoiceSignature,
    MinFinalCltvExpiry, PayeePubKey, PositiveTimestamp, PrivateRoute, RawDataPart, RawHrp, RawInvoice, RawTaggedField,
    SemanticError, Sha256, SiPrefix, SignedRawInvoice, TaggedField,
};
...
mod hrp_sm {
    ...
    impl StateMachine {
        ...
        fn step(&mut self, c: char) -> Result<(), super::ParseError> {
            ...
            match next_state {
                States::ParseCurrencyPrefix => StateMachine::update_range(&mut self.currency_prefix, self.position),
                States::ParseAmountNumber => StateMachine::update_range(&mut self.amount_number, self.position),
                States::ParseAmountSiPrefix => StateMachine::update_range(&mut self.amount_si_prefix, self.position),
                _ => {},
            }
...
impl FromStr for SignedRawInvoice {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        Ok(SignedRawInvoice {
            ...
            hash: RawInvoice::hash_from_parts(hrp.as_bytes(), &data[..data.len() - 104]),
...
impl FromBase32 for PositiveTimestamp {
    ...
    fn from_base32(b32: &[u5]) -> Result<Self, Self::Err> {
        if b32.len() != 7 {
            return Err(ParseError::InvalidSliceLength(
...
where
    T: CheckedAdd + CheckedMul + From<u8> + Default,
    U: Into<u8> + Copy,
{
    digits.iter().fold(Some(Default::default()), |acc, b| {
        acc.and_then(|x| x.checked_mul(&base))
...
impl FromBase32 for TaggedField {
    ...
    fn from_base32(field: &[u5]) -> Result<TaggedField, ParseError> {
        ...
        match tag.to_u8() {
            constants::TAG_PAYMENT_HASH => Ok(TaggedField::PaymentHash(Sha256::from_base32(field_data)?)),
            constants::TAG_DESCRIPTION => Ok(TaggedField::Description(Description::from_base32(field_data)?)),
            constants::TAG_PAYEE_PUB_KEY => Ok(TaggedField::PayeePubKey(PayeePubKey::from_base32(field_data)?)),
            constants::TAG_DESCRIPTION_HASH => Ok(TaggedField::DescriptionHash(Sha256::from_base32(field_data)?)),
            constants::TAG_EXPIRY_TIME => Ok(TaggedField::ExpiryTime(ExpiryTime::from_base32(field_data)?)),
            constants::TAG_MIN_FINAL_CLTV_EXPIRY => Ok(TaggedField::MinFinalCltvExpiry(
                MinFinalCltvExpiry::from_base32(field_data)?,
            )),
            constants::TAG_FALLBACK => Ok(TaggedField::Fallback(Fallback::from_base32(field_data)?)),
            constants::TAG_PRIVATE_ROUTE => Ok(TaggedField::PrivateRoute(PrivateRoute::from_base32(field_data)?)),
            constants::TAG_PAYMENT_SECRET => Ok(TaggedField::PaymentSecret(PaymentSecret::from_base32(field_data)?)),
            constants::TAG_FEATURES => Ok(TaggedField::Features(InvoiceFeatures::from_base32(field_data)?)),
            _ => {
...
impl FromBase32 for ExpiryTime {
    ...
    fn from_base32(field_data: &[u5]) -> Result<ExpiryTime, ParseError> {
        match parse_int_be::<u64, u5>(field_data, 32).map(|t| ExpiryTime::from_seconds(t)) {
            Some(t) => Ok(t),
...
impl Display for ParseError {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match *self {
            ...
            ParseError::UnexpectedEndOfTaggedFields => f.write_str("tagged fields part ended unexpectedly"),
            ParseError::PaddingError => f.write_str("some data field had bad padding"),
            ParseError::IntegerOverflowError => f.write_str("parsed integer doesn't fit into receiving type"),
            ParseError::InvalidSegWitProgramLength => f.write_str("fallback SegWit program is too long or too short"),
            ParseError::InvalidPubKeyHashLength => {
                f.write_str("fallback public key hash has a length unequal 20 bytes")
            },
            ParseError::InvalidScriptHashLength => f.write_str("fallback script hash has a length unequal 32 bytes"),
            ParseError::InvalidRecoveryId => f.write_str("recovery id is out of range (should be in [0,3])"),
            ParseError::Skip => {
...
mod test {
    ...
    fn test_parse_sha256_hash() {
        ...
        let hash = sha256::Hash::from_hex("0001020304050607080900010203040506070809000102030405060708090102").unwrap();
        let expected = Ok(Sha256(hash));

        assert_eq!(Sha256::from_base32(&input), expected);

        // make sure hashes of unknown length get skipped
        let input_unexpected_length = from_bech32("qqqsyqcyq5rqwzqfqqqsyqcyq5rqwzqfqqqsyqcyq5rqwzqfqypyq".as_bytes());
    ...
    fn test_parse_payee_pub_key() {
        ...
        let pk_bytes = [
            0x03, 0xe7, 0x15, 0x6a, 0xe3, 0x3b, 0x0a, 0x20, 0x8d, 0x07, 0x44, 0x19, 0x91, 0x63, 0x17, 0x7e, 0x90, 0x9e,
            0x80, 0x17, 0x6e, 0x55, 0xd9, 0x7a, 0x2f, 0x22, 0x1e, 0xde, 0x0f, 0x93, 0x4d, 0xd9, 0xad,
        ];
        ...
        // expects 33 bytes
        let input_unexpected_length = from_bech32("q0n326hr8v9zprg8gsvezcch06gfaqqhde2aj730yg0durunfhvq".as_bytes());
        assert_eq!(
        ...
        let cases = vec![
            (
                ...
                Ok(Fallback::PubKeyHash([
                    0x31, 0x72, 0xb5, 0x65, 0x4f, 0x66, 0x83, 0xc8, 0xfb, 0x14, 0x69, 0x59, 0xd3, 0x47, 0xce, 0x30,
                    0x3c, 0xae, 0x4c, 0xa7,
                ])),
            ),
            (
                ...
                Ok(Fallback::ScriptHash([
                    0x8f, 0x55, 0x56, 0x3b, 0x9a, 0x19, 0xf3, 0x21, 0xc2, 0x11, 0xe9, 0xb9, 0xf3, 0x8c, 0xdf, 0x68,
                    0x6e, 0xa0, 0x78, 0x45,
                ])),
            ),
            (
                ...
                Ok(Fallback::SegWitProgram {
                    ...
                    program: Vec::from(
                        &[
                            0x75u8, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94, 0x1c, 0x45, 0xd1, 0xb3, 0xa3,
                            0x23, 0xf1, 0x43, 0x3b, 0xd6,
                        ][..],
                    ),
                }),
            ),
            (vec![u5::try_from_u8(21).unwrap(); 41], Err(ParseError::Skip)),
            (vec![], Err(ParseError::UnexpectedEndOfTaggedFields)),
            (
    ...
    fn test_parse_route() {
        ...
        let input = from_bech32(
			...
			fqxu92d8lr6fvg0r5gv0heeeqgcrqlnm6jhphu9y00rrhy4grqszsvpcgpy9qqqqqqgqqqqq7qqzq"
                .as_bytes(),
        );
        ...
        expected.push(RouteHintHop {
            src_node_id: PublicKey::from_slice(
                &[
                    0x02u8, 0x9e, 0x03, 0xa9, 0x01, 0xb8, 0x55, 0x34, 0xff, 0x1e, 0x92, 0xc4, 0x3c, 0x74, 0x43, 0x1f,
                    0x7c, 0xe7, 0x20, 0x46, 0x06, 0x0f, 0xcf, 0x7a, 0x95, 0xc3, 0x7e, 0x14, 0x8f, 0x78, 0xc7, 0x72,
                    0x55,
                ][..],
            )
            .unwrap(),
            short_channel_id: parse_int_be(&[0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08], 256)
        ...
        expected.push(RouteHintHop {
            src_node_id: PublicKey::from_slice(
                &[
                    0x03u8, 0x9e, 0x03, 0xa9, 0x01, 0xb8, 0x55, 0x34, 0xff, 0x1e, 0x92, 0xc4, 0x3c, 0x74, 0x43, 0x1f,
                    0x7c, 0xe7, 0x20, 0x46, 0x06, 0x0f, 0xcf, 0x7a, 0x95, 0xc3, 0x7e, 0x14, 0x8f, 0x78, 0xc7, 0x72,
                    0x55,
                ][..],
            )
            .unwrap(),
            short_channel_id: parse_int_be(&[0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a], 256)
                .expect("short chan ID slice too big?"),
            fees: RoutingFees {
                base_msat: 2,
    ...
    fn test_payment_secret_and_features_de_and_ser() {
        ...
        use TaggedField::*;
        use {
            Currency, InvoiceSignature, PositiveTimestamp, RawDataPart, RawHrp, RawInvoice, Sha256, SiPrefix,
            SignedRawInvoice,
        };
        ...
        let invoice = SignedRawInvoice {
            ...
            hash: [
                0xb1, 0x96, 0x46, 0xc3, 0xbc, 0x56, 0x76, 0x1d, 0x20, 0x65, 0x6e, 0x0e, 0x32, 0xec, 0xd2, 0x69, 0x27,
                0xb7, 0x62, 0x6e, 0x2a, 0x8b, 0xe6, 0x97, 0x71, 0x9f, 0xf8, 0x7e, 0x44, 0x54, 0x55, 0xb9,
            ],
            signature: InvoiceSignature(
                RecoverableSignature::from_compact(
                    &[
                        0xd7, 0x90, 0x4c, 0xc4, 0xb7, 0x4a, 0x22, 0x26, 0x9c, 0x68, 0xc1, 0xdf, 0x68, 0xa9, 0x6c, 0x21,
                        0x4d, 0x65, 0x1b, 0x93, 0x76, 0xe9, 0xf1, 0x64, 0xd3, 0x60, 0x4d, 0xa4, 0xb7, 0xde, 0xcc, 0xce,
                        0x0e, 0x82, 0xaa, 0xab, 0x4c, 0x85, 0xd3, 0x58, 0xea, 0x14, 0xd0, 0xae, 0x34, 0x2d, 0xa3, 0x08,
                        0x12, 0xf9, 0x5d, 0x97, 0x60, 0x82, 0xea, 0xac, 0x81, 0x39, 0x11, 0xda, 0xe0, 0x1a, 0xf3, 0xc1,
                    ],
    ...
    fn test_raw_signed_invoice_deserialization() {
        ...
        use TaggedField::*;
        use {
            Currency, InvoiceSignature, PositiveTimestamp, RawDataPart, RawHrp, RawInvoice, Sha256, SignedRawInvoice,
        };
        ...
        assert_eq!(
            ...
            Ok(SignedRawInvoice {
                raw_invoice: RawInvoice {
                    ...
                    data: RawDataPart {
                        ...
                        tagged_fields: vec![
                            ...
                            .into(),
                            Description(
                                ::Description::new("Please consider supporting this project".to_owned()).unwrap()
                            )
                            .into(),
                        ],
                    },
                ...
                hash: [
                    0xc3, 0xd4, 0xe8, 0x3f, 0x64, 0x6f, 0xa7, 0x9a, 0x39, 0x3d, 0x75, 0x27, 0x7b, 0x1d, 0x85, 0x8d,
                    0xb1, 0xd1, 0xf7, 0xab, 0x71, 0x37, 0xdc, 0xb7, 0x83, 0x5d, 0xb2, 0xec, 0xd5, 0x18, 0xe1, 0xc9
                ],
                signature: InvoiceSignature(
                    RecoverableSignature::from_compact(
                        &[
                            0x38u8, 0xec, 0x68, 0x91, 0x34, 0x5e, 0x20, 0x41, 0x45, 0xbe, 0x8a, 0x3a, 0x99, 0xde, 0x38,
                            0xe9, 0x8a, 0x39, 0xd6, 0xa5, 0x69, 0x43, 0x4e, 0x18, 0x45, 0xc8, 0xaf, 0x72, 0x05, 0xaf,
                            0xcf, 0xcc, 0x7f, 0x42, 0x5f, 0xcd, 0x14, 0x63, 0xe9, 0x3c, 0x32, 0x88, 0x1e, 0xad, 0x0d,
                            0x6e, 0x35, 0x6d, 0x46, 0x7e, 0xc8, 0xc0, 0x25, 0x53, 0xf9, 0xaa, 0xb1, 0x5e, 0x57, 0x38,
                            0xb1, 0x1f, 0x12, 0x7f
                        ],
