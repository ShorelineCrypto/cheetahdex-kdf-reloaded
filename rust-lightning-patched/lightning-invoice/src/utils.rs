...
use {CreationError, Currency, Invoice, InvoiceBuilder, SignOrCreationError, DEFAULT_EXPIRY_TIME};
...
use lightning::ln::channelmanager::{
    ChannelDetails, ChannelManager, PaymentId, PaymentSendFailure, MIN_FINAL_CLTV_EXPIRY,
};
#[cfg(feature = "std")]
...
use lightning::routing::router::{find_route, Route, RouteHint, RouteHintHop, RouteParameters};
    ...
    K::Target: KeysInterface,
{
    ...
    let invoice = match description {
        InvoiceDescription::Direct(description) => InvoiceBuilder::new(network).description(description.0.clone()),
    ...
    let signed_raw_invoice =
        raw_invoice.sign(|_| keys_manager.sign_invoice(hrp_bytes, &data_without_signature, Recipient::PhantomNode));
    ...
    L::Target: Logger,
{
    _create_invoice_from_channelmanager_and_duration_since_epoch(
        ...
        network,
        amt_msat,
        InvoiceDescription::Direct(&Description::new(description).map_err(SignOrCreationError::CreationError)?),
        duration_since_epoch,
    )
}
    ...
    L::Target: Logger,
{
    ...
    let data_without_signature = raw_invoice.data.to_base32();
    let signed_raw_invoice =
        raw_invoice.sign(|_| keys_manager.sign_invoice(hrp_bytes, &data_without_signature, Recipient::Node));
    match signed_raw_invoice {
        Ok(inv) => Ok(Invoice::from_signed(inv).unwrap()),
        Err(e) => Err(SignOrCreationError::SignError(e)),
    }
...
pub struct DefaultRouter<G: Deref<Target = NetworkGraph>, L: Deref>
where
    L::Target: Logger,
{
    network_graph: G,
...
impl<G: Deref<Target = NetworkGraph>, L: Deref> DefaultRouter<G, L>
where
    L::Target: Logger,
{
...
mod test {
    ...
    fn test_from_channelmanager() {
        ...
        assert_eq!(
            invoice.description(),
            InvoiceDescription::Direct(&Description("test".to_string()))
        );
        ...
        let payment_event = {
            ...
            nodes[0]
                .node
                .send_payment(&route, payment_hash, &Some(invoice.payment_secret().clone()))
                .unwrap();
    ...
    fn test_create_invoice_with_description_hash() {
        ...
        assert_eq!(
            invoice.description(),
            InvoiceDescription::Hash(&crate::Sha256(Sha256::hash("Testing description_hash".as_bytes())))
        );
    }
    ...
    fn test_hints_includes_single_channels_to_nodes() {
        ...
        let chan_1_0 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            0,
            100000,
        ...
        let chan_2_0 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            2,
            0,
            100000,
    ...
    fn test_hints_has_only_highest_inbound_capacity_channel() {
        ...
        let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, None]);
        let nodes = create_network(2, &node_cfgs, &node_chanmgrs);
        let _chan_1_0_low_inbound_capacity = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            0,
            100_000,
        ...
        let chan_1_0_high_inbound_capacity = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            0,
            10_000_000,
        ...
        let _chan_1_0_medium_inbound_capacity = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            0,
            1_000_000,
    ...
    fn test_forwarding_info_not_assigned_channel_excluded_from_hints() {
        ...
        nodes[0]
            .node
            .handle_open_channel(&nodes[2].node.get_our_node_id(), InitFeatures::known(), &open_channel);
        ...
        nodes[2]
            .node
            .handle_accept_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &accept_channel);
        ...
        let as_funding_locked = get_event_msg!(
            nodes[2],
            MessageSendEvent::SendFundingLocked,
            nodes[0].node.get_our_node_id()
        );
        nodes[2].node.handle_funding_locked(
            &nodes[0].node.get_our_node_id(),
            &get_event_msg!(
                nodes[0],
                MessageSendEvent::SendFundingLocked,
        ...
        nodes[0]
            .node
            .handle_funding_locked(&nodes[2].node.get_our_node_id(), &as_funding_locked);
        get_event_msg!(
            nodes[0],
            MessageSendEvent::SendChannelUpdate,
            nodes[2].node.get_our_node_id()
    ...
    fn test_no_hints_if_a_mix_between_public_and_private_channel_exists() {
        ...
        let node_chanmgrs = create_node_chanmgrs(3, &node_cfgs, &[None, None, None]);
        let nodes = create_network(3, &node_cfgs, &node_chanmgrs);
        let _chan_1_0 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            0,
            100000,
        ...
        let chan_2_0 = create_announced_chan_between_nodes_with_value(
            &nodes,
            2,
            0,
            100000,
    ...
    fn test_only_public_channels_includes_no_channels_in_hints() {
        ...
        let node_chanmgrs = create_node_chanmgrs(3, &node_cfgs, &[None, None, None]);
        let nodes = create_network(3, &node_cfgs, &node_chanmgrs);
        let chan_1_0 = create_announced_chan_between_nodes_with_value(
            &nodes,
            1,
            0,
            100000,
    ...
    fn do_test_multi_node_receive(user_generated_pmt_hash: bool) {
        ...
        let (payment_preimage, payment_hash, payment_secret) = {
            if user_generated_pmt_hash {
                ...
                let payment_secret = nodes[1]
                    .node
                    .create_inbound_payment_for_hash(payment_hash, Some(payment_amt), 3600)
                    .unwrap();
                (payment_preimage, payment_hash, payment_secret)
            } else {
                let (payment_hash, payment_secret) =
                    nodes[1].node.create_inbound_payment(Some(payment_amt), 3600).unwrap();
                let payment_preimage = nodes[1]
                    .node
        ...
        let invoice = ::utils::create_phantom_invoice::<EnforcingSigner, &test_utils::TestKeysInterface>(
            Some(payment_amt),
            "test".to_string(),
            payment_hash,
            payment_secret,
    ...
    fn create_phantom_invoice_with_description_hash() {
        ...
        let invoice =
            ::utils::create_phantom_invoice_with_description_hash::<EnforcingSigner, &test_utils::TestKeysInterface>(
                Some(payment_amt),
                description_hash,
                payment_hash,
                payment_secret,
    ...
    fn test_multi_node_hints_includes_single_channels_to_participating_nodes() {
        ...
        let chan_0_1 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            0,
            1,
            100000,
        ...
        let chan_0_2 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            0,
            2,
            100000,
        ...
        match_multi_node_invoice_routes(Some(10_000), &nodes[1], vec![&nodes[1], &nodes[2]], scid_aliases, false);
    }
    ...
    fn test_multi_node_hints_includes_one_channel_of_each_counterparty_nodes_per_participating_node() {
        ...
        let chan_0_3 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            0,
            3,
            1000000,
        ...
        let chan_1_3 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            3,
            3_000_000,
        ...
        match_multi_node_invoice_routes(Some(10_000), &nodes[2], vec![&nodes[2], &nodes[3]], scid_aliases, false);
    }
    ...
    fn test_multi_node_forwarding_info_not_assigned_channel_excluded_from_hints() {
        ...
        nodes[3]
            .node
            .handle_open_channel(&nodes[1].node.get_our_node_id(), InitFeatures::known(), &open_channel);
        let accept_channel = get_event_msg!(
            nodes[3],
            MessageSendEvent::SendAcceptChannel,
        ...
        nodes[1]
            .node
            .handle_accept_channel(&nodes[3].node.get_our_node_id(), InitFeatures::known(), &accept_channel);
        ...
        nodes[3]
            .node
            .handle_funding_locked(&nodes[1].node.get_our_node_id(), &as_funding_locked);
        get_event_msg!(
            nodes[3],
            MessageSendEvent::SendChannelUpdate,
            nodes[1].node.get_our_node_id()
    ...
    fn test_multi_node_with_only_public_channels_hints_includes_only_phantom_route() {
        ...
        match_multi_node_invoice_routes(Some(10_000), &nodes[1], vec![&nodes[1], &nodes[2]], scid_aliases, true);
    }
    ...
    fn test_multi_node_with_mixed_public_and_private_channel_hints_includes_only_phantom_route() {
        ...
        nodes[2]
            .node
            .handle_channel_update(&nodes[0].node.get_our_node_id(), &chan_0_2.0);
        let _chan_1_2 = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            1,
            2,
            100000,
        ...
        match_multi_node_invoice_routes(Some(10_000), &nodes[2], vec![&nodes[2], &nodes[3]], scid_aliases, true);
    }
    ...
    fn test_multi_node_hints_has_only_highest_inbound_capacity_channel() {
        ...
        let _chan_0_1_low_inbound_capacity = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            0,
            1,
            100_000,
        ...
        let chan_0_1_high_inbound_capacity = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            0,
            1,
            10_000_000,
        ...
        let _chan_0_1_medium_inbound_capacity = create_unannounced_chan_between_nodes_with_value(
            &nodes,
            0,
            1,
            1_000_000,
