...
#[derive(Clone, Debug, Hash, Eq, PartialEq)]
pub enum Fallback {
    SegWitProgram { version: u5, program: Vec<u8> },
    PubKeyHash([u8; 20]),
    ScriptHash([u8; 20]),
}
...
impl<D: tb::Bool, H: tb::Bool, T: tb::Bool, C: tb::Bool, S: tb::Bool> InvoiceBuilder<D, H, T, C, S> {
    ...
    fn set_flags<DN: tb::Bool, HN: tb::Bool, TN: tb::Bool, CN: tb::Bool, SN: tb::Bool>(
        self,
    ...
    pub fn expiry_time(mut self, expiry_time: Duration) -> Self {
        self.tagged_fields
            .push(TaggedField::ExpiryTime(ExpiryTime::from_duration(expiry_time)));
        self
    }
...
impl<D: tb::Bool, H: tb::Bool, T: tb::Bool, S: tb::Bool> InvoiceBuilder<D, H, T, tb::False, S> {
    ...
    pub fn min_final_cltv_expiry(mut self, min_final_cltv_expiry: u64) -> InvoiceBuilder<D, H, T, tb::True, S> {
        self.tagged_fields
            .push(TaggedField::MinFinalCltvExpiry(MinFinalCltvExpiry(
...
impl InvoiceBuilder<tb::True, tb::True, tb::True, tb::True, tb::True> {
        ...
        F: FnOnce(&Message) -> Result<RecoverableSignature, E>,
    {
        ...
        let invoice = Invoice { signed_invoice: signed };
...
impl SignedRawInvoice {
    ...
    pub fn recover_payee_pub_key(&self) -> Result<PayeePubKey, secp256k1::Error> {
        let hash = Message::from_slice(&self.hash[..]).expect("Hash is 32 bytes long, same as MESSAGE_SIZE");

        Ok(PayeePubKey(Secp256k1::new().recover(&hash, &self.signature)?))
    }
    ...
    pub fn check_signature(&self) -> bool {
        ...
        let pub_key = included_pub_key
            .or_else(|| recovered_pub_key.as_ref())
            .expect("One is always present");

        let hash = Message::from_slice(&self.hash[..]).expect("Hash is 32 bytes long, same as MESSAGE_SIZE");

        let secp_context = Secp256k1::new();
        let verification_result = secp_context.verify(&hash, &self.signature.to_standard(), pub_key);
...
impl RawInvoice {
    ...
    pub fn hash(&self) -> [u8; 32] {
        ...
        RawInvoice::hash_from_parts(self.hrp.to_string().as_bytes(), &self.data.to_base32())
    }
        ...
        F: FnOnce(&Message) -> Result<RecoverableSignature, E>,
    {
        ...
        let hash = Message::from_slice(&raw_hash[..]).expect("Hash is 32 bytes long, same as MESSAGE_SIZE");
    ...
    pub fn known_tagged_fields(&self) -> FilterMap<Iter<RawTaggedField>, fn(&RawTaggedField) -> Option<&TaggedField>> {
...
impl Invoice {
    ...
    pub fn check_signature(&self) -> Result<(), SemanticError> {
        match self.signed_invoice.recover_payee_pub_key() {
            Err(secp256k1::Error::InvalidRecoveryId) => return Err(SemanticError::InvalidRecoveryId),
            Err(secp256k1::Error::InvalidSignature) => return Err(SemanticError::InvalidSignature),
    ...
    pub fn from_signed(signed_invoice: SignedRawInvoice) -> Result<Self, SemanticError> {
        let invoice = Invoice { signed_invoice };
    ...
    ///
    /// (C-not exported) As there is not yet a manual mapping for a FilterMap
    pub fn tagged_fields(&self) -> FilterMap<Iter<RawTaggedField>, fn(&RawTaggedField) -> Option<&TaggedField>> {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CreationError::TimestampOutOfBounds => {
                f.write_str("The Unix timestamp of the supplied date is less than zero or greater than 35-bits")
            },
            CreationError::InvalidAmount => {
                f.write_str("The supplied millisatoshi amount was greater than the total bitcoin supply")
            },
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SemanticError::MultipleDescriptions => {
                f.write_str("The invoice contains multiple descriptions and/or description hashes which isn't allowed")
            },
...
mod test {
    ...
    fn test_calc_invoice_hash() {
        use TaggedField::*;
        use {Currency, PositiveTimestamp, RawDataPart, RawHrp, RawInvoice};
        ...
        let invoice = RawInvoice {
            ...
            data: RawDataPart {
                ...
                tagged_fields: vec![
                    ...
                    .into(),
                    Description(::Description::new("Please consider supporting this project".to_owned()).unwrap())
                        .into(),
                ],
            },
        };
        ...
        let expected_hash = [
            0xc3, 0xd4, 0xe8, 0x3f, 0x64, 0x6f, 0xa7, 0x9a, 0x39, 0x3d, 0x75, 0x27, 0x7b, 0x1d, 0x85, 0x8d, 0xb1, 0xd1,
            0xf7, 0xab, 0x71, 0x37, 0xdc, 0xb7, 0x83, 0x5d, 0xb2, 0xec, 0xd5, 0x18, 0xe1, 0xc9,
        ];
    ...
    fn test_check_signature() {
        ...
        let invoice = SignedRawInvoice {
            ...
            hash: [
                0xc3, 0xd4, 0xe8, 0x3f, 0x64, 0x6f, 0xa7, 0x9a, 0x39, 0x3d, 0x75, 0x27, 0x7b, 0x1d, 0x85, 0x8d, 0xb1,
                0xd1, 0xf7, 0xab, 0x71, 0x37, 0xdc, 0xb7, 0x83, 0x5d, 0xb2, 0xec, 0xd5, 0x18, 0xe1, 0xc9,
            ],
            signature: InvoiceSignature(
                RecoverableSignature::from_compact(
                    &[
                        0x38u8, 0xec, 0x68, 0x91, 0x34, 0x5e, 0x20, 0x41, 0x45, 0xbe, 0x8a, 0x3a, 0x99, 0xde, 0x38,
                        0xe9, 0x8a, 0x39, 0xd6, 0xa5, 0x69, 0x43, 0x4e, 0x18, 0x45, 0xc8, 0xaf, 0x72, 0x05, 0xaf, 0xcf,
                        0xcc, 0x7f, 0x42, 0x5f, 0xcd, 0x14, 0x63, 0xe9, 0x3c, 0x32, 0x88, 0x1e, 0xad, 0x0d, 0x6e, 0x35,
                        0x6d, 0x46, 0x7e, 0xc8, 0xc0, 0x25, 0x53, 0xf9, 0xaa, 0xb1, 0x5e, 0x57, 0x38, 0xb1, 0x1f, 0x12,
                        0x7f,
                    ],
        ...
        let private_key = SecretKey::from_slice(
            &[
                0xe1, 0x26, 0xf6, 0x8f, 0x7e, 0xaf, 0xcc, 0x8b, 0x74, 0xf5, 0x4d, 0x26, 0x9f, 0xe2, 0x06, 0xbe, 0x71,
                0x50, 0x00, 0xf9, 0x4d, 0xac, 0x06, 0x7d, 0x1c, 0x04, 0xa8, 0xca, 0x3b, 0x2d, 0xb7, 0x34,
            ][..],
        )
        .unwrap();
        ...
        let new_signed = raw_invoice
            .sign::<_, ()>(|hash| Ok(Secp256k1::new().sign_recoverable(hash, &private_key)))
            .unwrap();
    ...
    fn test_check_feature_bits() {
        ...
        use secp256k1::Secp256k1;
        use TaggedField::*;
        use {Currency, Invoice, PositiveTimestamp, RawDataPart, RawHrp, RawInvoice, SemanticError, Sha256};
        ...
        let invoice = {
            ...
            invoice
                .data
                .tagged_fields
                .push(Features(InvoiceFeatures::empty()).into());
            invoice.sign::<_, ()>(|hash| Ok(Secp256k1::new().sign_recoverable(hash, &private_key)))
        }
        .unwrap();
        assert_eq!(Invoice::from_signed(invoice), Err(SemanticError::InvalidFeatures));
        ...
        let invoice = {
            ...
            invoice
                .data
                .tagged_fields
                .push(Features(InvoiceFeatures::known()).into());
            invoice.sign::<_, ()>(|hash| Ok(Secp256k1::new().sign_recoverable(hash, &private_key)))
        }
        .unwrap();
        ...
        .unwrap();
        assert_eq!(
            Invoice::from_signed(invoice),
            Err(SemanticError::MultiplePaymentSecrets)
        );
    }
    ...
    fn test_builder_amount() {
        ...
        let invoice = builder.clone().amount_milli_satoshis(150).build_raw().unwrap();
    ...
    fn test_builder_fail() {
        ...
        let route_hop = RouteHintHop {
            src_node_id: PublicKey::from_slice(
                &[
                    0x03, 0x9e, 0x03, 0xa9, 0x01, 0xb8, 0x55, 0x34, 0xff, 0x1e, 0x92, 0xc4, 0x3c, 0x74, 0x43, 0x1f,
    ...
    fn test_builder_ok() {
        ...
        let invoice = builder
            .clone()
            .build_signed(|hash| secp_ctx.sign_recoverable(hash, &private_key))
            .unwrap();
