    ...
    L::Target: Logger,
{
    ...
    pub fn new(payer: P, router: R, scorer: S, logger: L, event_handler: E, retry_attempts: RetryAttempts) -> Self {
        Self {
            payer,
            router,
            scorer,
    ...
    /// never been paid before. Because [`InvoicePayer`] is stateless no effort is made to do so
    /// for you.
    pub fn pay_zero_value_invoice(&self, invoice: &Invoice, amount_msats: u64) -> Result<PaymentId, PaymentError> {
    ...
    fn pay_invoice_using_amount(
        ...
        let send_payment = |route: &Route| self.payer.send_payment(route, payment_hash, &payment_secret);
    ...
    pub fn pay_pubkey(
        ...
        let send_payment = |route: &Route| self.payer.send_spontaneous_payment(route, payment_preimage);
        self.pay_internal(&route_params, payment_hash, send_payment)
            .map_err(|e| {
                self.payment_cache.lock().unwrap().remove(&payment_hash);
                e
    ...
    fn retry_payment(
        ...
        match self.payer.retry_payment(&route.unwrap(), payment_id) {
            Ok(()) => Ok(()),
            Err(PaymentSendFailure::ParameterError(_)) | Err(PaymentSendFailure::PathParameterError(_)) => {
                log_trace!(
                    self.logger,
            ...
            Err(PaymentSendFailure::AllFailedRetrySafe(_)) => self.retry_payment(payment_id, payment_hash, params),
    ...
    fn handle_event(&self, event: &Event) {
        match event {
            ...
            Event::PaymentSent { payment_hash, .. } => {
                let mut payment_cache = self.payment_cache.lock().unwrap();
                let attempts = payment_cache.remove(payment_hash).map_or(1, |attempts| attempts + 1);
                log_trace!(
                    self.logger,
...
mod tests {
    ...
    fn pays_invoice_on_first_attempt() {
        ...
        let invoice_payer = InvoicePayer::new(&payer, router, &scorer, &logger, event_handler, RetryAttempts(0));
    ...
    fn fails_paying_invoice_with_routing_errors() {
        ...
        let scorer = RefCell::new(TestScorer::new());
        let logger = TestLogger::new();
        let invoice_payer = InvoicePayer::new(&payer, router, &scorer, &logger, |_: &_| {}, RetryAttempts(0));
    ...
    fn pays_zero_value_invoice_using_amount() {
        ...
        let payment_id = Some(
            invoice_payer
                .pay_zero_value_invoice(&invoice, final_value_msat)
                .unwrap(),
        );
        assert_eq!(*payer.attempts.borrow(), 1);
    ...
    fn scores_successful_channels() {
        ...
        let scorer = RefCell::new(
            TestScorer::new()
                .expect(PaymentPath::Success {
                    path: route.paths[0].clone(),
                })
                .expect(PaymentPath::Success {
                    path: route.paths[1].clone(),
                }),
        );
        let logger = TestLogger::new();
        let invoice_payer = InvoicePayer::new(&payer, router, &scorer, &logger, event_handler, RetryAttempts(2));
    ...
    impl Payer for TestPayer {
        ...
        fn retry_payment(&self, route: &Route, _payment_id: PaymentId) -> Result<(), PaymentSendFailure> {
    ...
    fn no_extra_retries_on_back_to_back_fail() {
        ...
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_first_cs);
        check_added_monitors!(nodes[0], 1);
        ...
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &second_htlc_updates.commitment_msg);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[0]
            .node
            .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_second_raa);
        check_added_monitors!(nodes[0], 1);

        nodes[0]
            .node
            .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &bs_fail_update.update_fail_htlcs[0]);
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_fail_update.commitment_signed);
        check_added_monitors!(nodes[0], 1);
        ...
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_third_cs);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[1]
            .node
            .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_third_raa);
        check_added_monitors!(nodes[1], 1);
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_fourth_cs);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[0]
            .node
            .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_fourth_raa);
        check_added_monitors!(nodes[0], 1);
