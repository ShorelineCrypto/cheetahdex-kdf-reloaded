...
use super::{
    constants, Currency, Description, ExpiryTime, Fallback, Invoice, InvoiceSignature, MinFinalCltvExpiry, PayeePubKey,
    PositiveTimestamp, PrivateRoute, RawDataPart, RawHrp, RawTaggedField, Sha256, SiPrefix, SignedRawInvoice,
    TaggedField,
};
...
impl Base32Len for Fallback {
    fn base32_len(&self) -> usize {
        match *self {
            Fallback::SegWitProgram { program: ref p, .. } => bytes_size_to_base32_size(p.len()) + 1,
    ...
    fn write_base32<W: WriteBase32>(&self, writer: &mut W) -> Result<(), <W as WriteBase32>::Err> {
        ...
        for hop in (self.0).0.iter() {
            ...
            let short_channel_id =
                try_stretch(encode_int_be_base256(hop.short_channel_id), 8).expect("sizeof(u64) == 8");
            converter.append(&short_channel_id)?;

            let fee_base_msat = try_stretch(encode_int_be_base256(hop.fees.base_msat), 4).expect("sizeof(u32) == 4");
            converter.append(&fee_base_msat)?;

            let fee_proportional_millionths =
                try_stretch(encode_int_be_base256(hop.fees.proportional_millionths), 4).expect("sizeof(u32) == 4");
            converter.append(&fee_proportional_millionths)?;

            let cltv_expiry_delta =
                try_stretch(encode_int_be_base256(hop.cltv_expiry_delta), 2).expect("sizeof(u16) == 2");
...
impl ToBase32 for TaggedField {
    fn write_base32<W: WriteBase32>(&self, writer: &mut W) -> Result<(), <W as WriteBase32>::Err> {
            ...
            P: ToBase32 + Base32Len,
        {
            ...
            writer.write(
                &try_stretch(encode_int_be_base32(len as u64), 2).expect("Can't be longer than 2, see assert above."),
            )?;
            payload.write_base32(writer)
        }

        match *self {
            TaggedField::PaymentHash(ref hash) => write_tagged_field(writer, constants::TAG_PAYMENT_HASH, hash),
            TaggedField::Description(ref description) => {
                write_tagged_field(writer, constants::TAG_DESCRIPTION, description)
            },
            TaggedField::PayeePubKey(ref pub_key) => write_tagged_field(writer, constants::TAG_PAYEE_PUB_KEY, pub_key),
            TaggedField::DescriptionHash(ref hash) => write_tagged_field(writer, constants::TAG_DESCRIPTION_HASH, hash),
            TaggedField::ExpiryTime(ref duration) => write_tagged_field(writer, constants::TAG_EXPIRY_TIME, duration),
            ...
            TaggedField::Features(ref features) => write_tagged_field(writer, constants::TAG_FEATURES, features),
        }
    }
}
