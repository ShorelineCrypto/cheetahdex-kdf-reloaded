...
use ln::msgs;
use ln::msgs::{ChannelAnnouncement, ChannelUpdate, GossipTimestampFilter, NodeAnnouncement, OptionalField};
use ln::msgs::{DecodeError, ErrorAction, Init, LightningError, NetAddress, RoutingMessageHandler, MAX_VALUE_MSAT};
use ln::msgs::{QueryChannelRange, QueryShortChannelIds, ReplyChannelRange, ReplyShortChannelIdsEnd};
use util::events::{Event, EventHandler, MessageSendEvent, MessageSendEventsProvider};
use util::logger::{Level, Logger};
use util::scid_utils::{block_from_scid, scid_from_parts, MAX_SCID_BLOCK};
use util::ser::{Readable, Writeable, Writer};

use alloc::collections::{btree_map::Entry as BtreeEntry, BTreeMap};
    ...
    fn handle_network_update(&self, update: &NetworkUpdate) {
        match *update {
            NetworkUpdate::ChannelUpdateMessage { ref msg } => {
                ...
                log_debug!(
                    self.logger,
                    "Updating channel with channel_update from a payment failure. Channel {} is {}.",
                    short_channel_id,
                    status
                );
            ...
            NetworkUpdate::ChannelClosed {
                ...
                log_debug!(
                    self.logger,
                    "{} channel graph entry for {} due to a payment failure.",
                    action,
                    short_channel_id
                );
                self.network_graph
            ...
            NetworkUpdate::NodeFailure {
                ...
                log_debug!(
                    self.logger,
                    "{} node graph entry for {} due to a payment failure.",
                    action,
                    node_id
                );
    ...
    L::Target: Logger,
{
    ...
    fn handle_channel_announcement(&self, msg: &msgs::ChannelAnnouncement) -> Result<bool, LightningError> {
        self.network_graph
            .update_channel_from_announcement(msg, &self.chain_access, &self.secp_ctx)?;
        log_gossip!(
            self.logger,
            "Added channel_announcement for {}{}",
            msg.contents.short_channel_id,
            if !msg.contents.excess_data.is_empty() {
    ...
    fn handle_query_channel_range(
        ...
        if msg.chain_hash != self.network_graph.genesis_hash
...
impl fmt::Display for ChannelInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            ...
            log_bytes!(self.node_one.as_slice()),
            self.one_to_two,
            log_bytes!(self.node_two.as_slice()),
            self.two_to_one
        )?;
        Ok(())
    }
...
impl<'a> DirectedChannelInfo<'a> {
    ...
    pub fn direction(&self) -> Option<&'a ChannelUpdateInfo> {
        self.direction
    }
            ...
            .map(|max_htlc_msat| {
                ...
                if max_htlc_msat < capacity_msat {
                    EffectiveCapacity::MaximumHTLC {
                        amount_msat: max_htlc_msat,
                    }
                } else {
                    EffectiveCapacity::Total { capacity_msat }
                }
            })
            .or_else(|| capacity_msat.map(|capacity_msat| EffectiveCapacity::Total { capacity_msat }))
...
impl<'a> DirectedChannelInfoWithUpdate<'a> {
    /// Returns information for the channel.
    #[inline]
    pub(super) fn channel(&self) -> &'a ChannelInfo {
        &self.inner.channel
    }
    ...
    pub(super) fn direction(&self) -> &'a ChannelUpdateInfo {
        self.inner.direction.unwrap()
    }

    /// Returns the [`EffectiveCapacity`] of the channel in the direction.
    #[inline]
    pub(super) fn effective_capacity(&self) -> EffectiveCapacity {
...
impl fmt::Display for NodeInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            f,
            "lowest_inbound_channel_fees: {:?}, channels: {:?}, announcement_info: {:?}",
...
impl NetworkGraph {
    ...
    pub fn update_node_from_announcement<T: secp256k1::Verification>(
        &self,
    ...
    pub fn update_channel<T: secp256k1::Verification>(
        &self,
    ...
    pub fn update_channel_unsigned(&self, msg: &msgs::UnsignedChannelUpdate) -> Result<(), LightningError> {
        self.update_channel_intern(
            msg,
            None,
            None::<(&secp256k1::Signature, &Secp256k1<secp256k1::VerifyOnly>)>,
        )
    }

    fn update_channel_intern<T: secp256k1::Verification>(
        &self,
        msg: &msgs::UnsignedChannelUpdate,
        full_msg: Option<&msgs::ChannelUpdate>,
        sig_info: Option<(&secp256k1::Signature, &Secp256k1<T>)>,
    ) -> Result<(), LightningError> {
        let dest_node_id;
                ...
                if let OptionalField::Present(htlc_maximum_msat) = msg.htlc_maximum_msat {
                    ...
                    if let Some(capacity_sats) = channel.capacity_sats {
                        ...
                        if capacity_sats > MAX_VALUE_MSAT / 1000 || htlc_maximum_msat > capacity_sats * 1000 {
                            return Err(LightningError {
                                err: "htlc_maximum_msat is larger than channel capacity or capacity is bogus"
                                    .to_owned(),
                                action: ErrorAction::IgnoreError,
                            });
                        }
                ...
                macro_rules! get_new_channel_info {
                    () => {{
                        ...
                        let updated_channel_update_info = ChannelUpdateInfo {
                            ...
                            htlc_maximum_msat: if let OptionalField::Present(max_value) = msg.htlc_maximum_msat {
                                Some(max_value)
                            } else {
                                None
                            },
                ...
                if msg.flags & 1 == 1 {
                    ...
                    if let Some((sig, ctx)) = sig_info {
                        secp_verify_sig!(
                            ...
                            &msg_hash,
                            &sig,
                            &PublicKey::from_slice(channel.node_two.as_slice()).map_err(|_| LightningError {
                    ...
                    if let Some((sig, ctx)) = sig_info {
                        secp_verify_sig!(
                            ...
                            &msg_hash,
                            &sig,
                            &PublicKey::from_slice(channel.node_one.as_slice()).map_err(|_| LightningError {
                ...
                if let Some(chan_info) = chan_info_opt {
                    if chan_info.enabled {
                        ...
                        fees.proportional_millionths =
                            cmp::min(fees.proportional_millionths, chan_info.fees.proportional_millionths);
                    }
                }
            }
...
mod tests {
    ...
    use ln::msgs::{
        ChannelAnnouncement, ChannelUpdate, Init, NodeAnnouncement, OptionalField, QueryChannelRange,
        QueryShortChannelIds, ReplyChannelRange, RoutingMessageHandler, UnsignedChannelAnnouncement,
    ...
    fn create_net_graph_msg_handler(
        network_graph: &NetworkGraph,
    ) -> (
        Secp256k1<All>,
        NetGraphMsgHandler<&NetworkGraph, Arc<test_utils::TestChainSource>, Arc<test_utils::TestLogger>>,
    ) {
    ...
    fn get_signed_node_announcement<F: Fn(&mut UnsignedNodeAnnouncement)>(
        f: F,
        node_key: &SecretKey,
    ...
    fn get_signed_channel_announcement<F: Fn(&mut UnsignedChannelAnnouncement)>(
        f: F,
    ...
    fn get_signed_channel_update<F: Fn(&mut UnsignedChannelUpdate)>(
        f: F,
        node_key: &SecretKey,
        secp_ctx: &Secp256k1<secp256k1::All>,
    ) -> ChannelUpdate {
    ...
    fn handling_node_announcements() {
        ...
        match net_graph_msg_handler.handle_node_announcement(&NodeAnnouncement {
    ...
    fn handling_channel_announcements() {
        ...
        *chain_source.utxo_ret.lock().unwrap() = Err(chain::AccessError::UnknownTx);
        ...
        net_graph_msg_handler =
            NetGraphMsgHandler::new(&network_graph, Some(chain_source.clone()), Arc::clone(&logger));
        ...
        *chain_source.utxo_ret.lock().unwrap() = Ok(TxOut {
            value: 0,
    ...
    fn handling_network_update() {
        ...
        let short_channel_id;
        {
            ...
            assert!(network_graph
                .update_channel_from_announcement(&valid_channel_announcement, &chain_source, &secp_ctx)
                .is_ok());
    ...
    fn test_channel_timeouts() {
        ...
        let valid_channel_update = get_signed_channel_update(|_| {}, node_1_privkey, &secp_ctx);
        assert!(net_graph_msg_handler
            .handle_channel_update(&valid_channel_update)
            .is_ok());
        assert!(network_graph
            .read_only()
            .channels()
    ...
    fn getting_next_channel_announcements() {
        ...
        let channels_with_announcements =
            net_graph_msg_handler.get_next_channel_announcements(short_channel_id + 1000, 1);
        assert_eq!(channels_with_announcements.len(), 0);
    }
    ...
    fn calling_sync_routing_table() {
        ...
        {
            let init_msg = Init {
                features: InitFeatures::known().clear_gossip_queries(),
                ...
                MessageSendEvent::SendReplyChannelRange { node_id, msg } => {
                    ...
                    assert!(
                        msg.first_blocknum == c_lightning_0_9_prev_end_blocknum
                            || msg.first_blocknum == c_lightning_0_9_prev_end_blocknum.saturating_add(1)
                    );
    ...
    fn handling_query_short_channel_ids() {
        ...
        let result = net_graph_msg_handler.handle_query_short_channel_ids(
            &node_id,
