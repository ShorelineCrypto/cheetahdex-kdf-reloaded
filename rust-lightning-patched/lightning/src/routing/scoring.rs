...
#[deprecated(since = "0.0.105", note = "ProbabilisticScorer should be used instead of Scorer.")]
...
impl<T: Time> Score for ScorerUsingTime<T> {
    fn channel_penalty_msat(
        ...
        let failure_penalty_msat = self.channel_failures.get(&short_channel_id).map_or(0, |value| {
            value.decayed_penalty_msat(self.params.failure_penalty_half_life)
        });
        ...
        if send_1024ths > self.params.overuse_penalty_start_1024th as u64 {
            penalty_msat = penalty_msat
                .checked_add(
                    (send_1024ths - self.params.overuse_penalty_start_1024th as u64)
                        .checked_mul(self.params.overuse_penalty_msat_per_1024th)
...
impl<L: Deref<Target = u64>, T: Time, U: Deref<Target = T>> DirectedChannelLiquidity<L, T, U> {
    ...
    fn penalty_msat(&self, amount_msat: u64, params: ProbabilisticScoringParameters) -> u64 {
        ...
        if amount_msat <= min_liquidity_msat {
            ...
            let negative_log10_times_1024 = approx::negative_log10_times_1024(numerator, denominator);
            self.combined_penalty_msat(amount_msat, negative_log10_times_1024, params)
        }
    }
...
mod approx {
    ...
    const LOG10_TIMES_1024: [[u16; LOWER_BITS_BOUND as usize]; BITS as usize] = [
        ...
        [
            10172, 10199, 10225, 10249, 10272, 10293, 10314, 10334, 10353, 10371, 10388, 10405, 10421, 10437, 10452,
            10467,
        ],
        [
            10481, 10508, 10533, 10557, 10580, 10602, 10622, 10642, 10661, 10679, 10697, 10713, 10730, 10745, 10760,
            10775,
        ],
        [
            10789, 10816, 10841, 10865, 10888, 10910, 10931, 10950, 10969, 10987, 11005, 11022, 11038, 11053, 11068,
            11083,
        ],
        [
            11097, 11124, 11150, 11174, 11196, 11218, 11239, 11259, 11277, 11296, 11313, 11330, 11346, 11362, 11377,
            11391,
        ],
        [
            11405, 11432, 11458, 11482, 11505, 11526, 11547, 11567, 11586, 11604, 11621, 11638, 11654, 11670, 11685,
            11700,
        ],
        [
            11714, 11741, 11766, 11790, 11813, 11835, 11855, 11875, 11894, 11912, 11930, 11946, 11963, 11978, 11993,
            12008,
        ],
        [
            12022, 12049, 12074, 12098, 12121, 12143, 12164, 12183, 12202, 12220, 12238, 12255, 12271, 12286, 12301,
            12316,
        ],
        [
            12330, 12357, 12383, 12407, 12429, 12451, 12472, 12492, 12511, 12529, 12546, 12563, 12579, 12595, 12610,
            12624,
        ],
        [
            12638, 12665, 12691, 12715, 12738, 12759, 12780, 12800, 12819, 12837, 12854, 12871, 12887, 12903, 12918,
            12933,
        ],
        [
            12947, 12974, 12999, 13023, 13046, 13068, 13088, 13108, 13127, 13145, 13163, 13179, 13196, 13211, 13226,
            13241,
        ],
        [
            13255, 13282, 13307, 13331, 13354, 13376, 13397, 13416, 13435, 13453, 13471, 13488, 13504, 13519, 13535,
            13549,
        ],
        [
            13563, 13590, 13616, 13640, 13662, 13684, 13705, 13725, 13744, 13762, 13779, 13796, 13812, 13828, 13843,
            13857,
        ],
        [
            13871, 13898, 13924, 13948, 13971, 13992, 14013, 14033, 14052, 14070, 14087, 14104, 14120, 14136, 14151,
            14166,
        ],
        [
            14180, 14207, 14232, 14256, 14279, 14301, 14321, 14341, 14360, 14378, 14396, 14412, 14429, 14444, 14459,
            14474,
        ],
        [
            14488, 14515, 14540, 14564, 14587, 14609, 14630, 14649, 14668, 14686, 14704, 14721, 14737, 14752, 14768,
            14782,
        ],
        [
            14796, 14823, 14849, 14873, 14895, 14917, 14938, 14958, 14977, 14995, 15012, 15029, 15045, 15061, 15076,
            15090,
        ],
        [
            15104, 15131, 15157, 15181, 15204, 15225, 15246, 15266, 15285, 15303, 15320, 15337, 15353, 15369, 15384,
            15399,
        ],
        [
            15413, 15440, 15465, 15489, 15512, 15534, 15554, 15574, 15593, 15611, 15629, 15645, 15662, 15677, 15692,
            15707,
        ],
        [
            15721, 15748, 15773, 15797, 15820, 15842, 15863, 15882, 15901, 15919, 15937, 15954, 15970, 15985, 16001,
            16015,
        ],
        [
            16029, 16056, 16082, 16106, 16128, 16150, 16171, 16191, 16210, 16228, 16245, 16262, 16278, 16294, 16309,
            16323,
        ],
        [
            16337, 16364, 16390, 16414, 16437, 16458, 16479, 16499, 16518, 16536, 16553, 16570, 16586, 16602, 16617,
            16632,
        ],
        [
            16646, 16673, 16698, 16722, 16745, 16767, 16787, 16807, 16826, 16844, 16862, 16878, 16895, 16910, 16925,
            16940,
        ],
        [
            16954, 16981, 17006, 17030, 17053, 17075, 17096, 17115, 17134, 17152, 17170, 17187, 17203, 17218, 17234,
            17248,
        ],
        [
            17262, 17289, 17315, 17339, 17361, 17383, 17404, 17424, 17443, 17461, 17478, 17495, 17511, 17527, 17542,
            17556,
        ],
        [
            17571, 17597, 17623, 17647, 17670, 17691, 17712, 17732, 17751, 17769, 17786, 17803, 17819, 17835, 17850,
            17865,
        ],
        [
            17879, 17906, 17931, 17955, 17978, 18000, 18020, 18040, 18059, 18077, 18095, 18111, 18128, 18143, 18158,
            18173,
        ],
        [
            18187, 18214, 18239, 18263, 18286, 18308, 18329, 18348, 18367, 18385, 18403, 18420, 18436, 18452, 18467,
            18481,
        ],
        [
            18495, 18522, 18548, 18572, 18595, 18616, 18637, 18657, 18676, 18694, 18711, 18728, 18744, 18760, 18775,
            18789,
        ],
        [
            18804, 18830, 18856, 18880, 18903, 18924, 18945, 18965, 18984, 19002, 19019, 19036, 19052, 19068, 19083,
            19098,
        ],
        [
            19112, 19139, 19164, 19188, 19211, 19233, 19253, 19273, 19292, 19310, 19328, 19344, 19361, 19376, 19391,
            19406,
        ],
        [
            19420, 19447, 19472, 19496, 19519, 19541, 19562, 19581, 19600, 19619, 19636, 19653, 19669, 19685, 19700,
            19714,
        ],
    ];
...
impl<G: Deref<Target = NetworkGraph>, T: Time> Writeable for ProbabilisticScorerUsingTime<G, T> {
    #[inline]
    fn write<W: Writer>(&self, w: &mut W) -> Result<(), io::Error> {
        write_tlv_fields!(w, { (0, self.channel_liquidities, required) });
        Ok(())
    }
}

impl<G: Deref<Target = NetworkGraph>, T: Time> ReadableArgs<(ProbabilisticScoringParameters, G)>
    for ProbabilisticScorerUsingTime<G, T>
{
    #[inline]
    fn read<R: Read>(r: &mut R, args: (ProbabilisticScoringParameters, G)) -> Result<Self, DecodeError> {
        let (params, network_graph) = args;
        let mut channel_liquidities = HashMap::new();
        read_tlv_fields!(r, { (0, channel_liquidities, required) });
        Ok(Self {
            params,
            network_graph,
            channel_liquidities,
...
mod tests {
    ...
    use super::{
        ChannelLiquidity, ProbabilisticScorerUsingTime, ProbabilisticScoringParameters, ScorerUsingTime,
        ScoringParameters, Time,
    };

    use ln::features::{ChannelFeatures, NodeFeatures};
    use ln::msgs::{
        ChannelAnnouncement, ChannelUpdate, OptionalField, UnsignedChannelAnnouncement, UnsignedChannelUpdate,
    };
    use routing::network_graph::{NetworkGraph, NodeId};
    use routing::router::RouteHop;
    ...
    fn restores_persisted_channel_failure_penalties() {
        ...
        assert_eq!(
            deserialized_scorer.channel_penalty_msat(42, 1, 1, &source, &target),
            1_256
        );
        assert_eq!(
            deserialized_scorer.channel_penalty_msat(43, 1, 1, &source, &target),
            1_512
        );
    }
    ...
    fn charges_per_1024th_penalty() {
        ...
        assert_eq!(
            scorer.channel_penalty_msat(42, 257_000, 1_024_000, &source, &target),
            100
        );
        assert_eq!(
            scorer.channel_penalty_msat(42, 258_000, 1_024_000, &source, &target),
            200
        );
        assert_eq!(
            scorer.channel_penalty_msat(42, 512_000, 1_024_000, &source, &target),
            256 * 100
        );
    }
    ...
    fn add_channel(
        network_graph: &mut NetworkGraph,
        short_channel_id: u64,
        ...
        network_graph
            .update_channel_from_announcement(&signed_announcement, &chain_source, &secp_ctx)
            .unwrap();
        update_channel(network_graph, short_channel_id, node_1_key, 0);
        update_channel(network_graph, short_channel_id, node_2_key, 1);
    }

    fn update_channel(network_graph: &mut NetworkGraph, short_channel_id: u64, node_key: SecretKey, flags: u8) {
        let genesis_hash = genesis_block(Network::Testnet).header.block_hash();
        let secp_ctx = Secp256k1::new();
    ...
    fn liquidity_bounds_directed_from_lowest_node_id() {
        ...
        let liquidity = scorer.channel_liquidities.get(&42).unwrap().as_directed(
            &source,
            &target,
            1_000,
        ...
        let liquidity = scorer.channel_liquidities.get(&43).unwrap().as_directed(
            &target,
            &recipient,
            1_000,
            liquidity_offset_half_life,
    ...
    fn resets_liquidity_upper_bound_when_crossed_by_lower_bound() {
        ...
        let network_graph = network_graph();
        let params = ProbabilisticScoringParameters::default();
        let mut scorer = ProbabilisticScorer::new(params, &network_graph).with_channel(
            42,
            ChannelLiquidity {
    ...
    fn increased_penalty_nearing_liquidity_upper_bound() {
        ...
        assert_eq!(
            scorer.channel_penalty_msat(42, 102_400, 1_024_000, &source, &target),
            43
        );
        assert_eq!(
            scorer.channel_penalty_msat(42, 1_024_000, 1_024_000, &source, &target),
            2_000
        );
    ...
    fn constant_penalty_outside_liquidity_bounds() {
        ...
        let scorer = ProbabilisticScorer::new(params, &network_graph).with_channel(
            42,
            ChannelLiquidity {
        ...
        assert_ne!(
            scorer.channel_penalty_msat(42, 50, 100, &source, &target),
            u64::max_value()
        );
        assert_eq!(
            scorer.channel_penalty_msat(42, 61, 100, &source, &target),
            u64::max_value()
        );
    }
    ...
    fn sets_liquidity_upper_bound_on_failure() {
        ...
        assert_eq!(
            scorer.channel_penalty_msat(42, 500, 1_000, &source, &target),
            u64::max_value()
        );
        assert_eq!(
            scorer.channel_penalty_msat(42, 750, 1_000, &source, &target),
            u64::max_value()
        );
    }
    ...
    fn decays_liquidity_bounds_over_time() {
        ...
        assert_eq!(
            scorer.channel_penalty_msat(42, 896, 1_024, &source, &target),
            u64::max_value()
        );
        ...
        assert_eq!(
            scorer.channel_penalty_msat(42, 960, 1_024, &source, &target),
            u64::max_value()
        );
    ...
    fn restores_persisted_liquidity_bounds() {
        ...
        assert_eq!(
            deserialized_scorer.channel_penalty_msat(42, 500, 1_000, &source, &target),
            300
        );
    }
