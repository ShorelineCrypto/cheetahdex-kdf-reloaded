...
use routing::network_graph::{
    DirectedChannelInfoWithUpdate, EffectiveCapacity, NetworkGraph, NodeId, ReadOnlyNetworkGraph, RoutingFees,
};
...
enum CandidateRouteHop<'a> {
    ...
    FirstHop { details: &'a ChannelDetails },
    ...
    PrivateHop { hint: &'a RouteHintHop },
}
...
impl<'a> PaymentPath<'a> {
    ...
    fn update_value_and_recompute_fees(&mut self, value_msat: u64) {
        ...
        for i in (0..self.hops.len()).rev() {
            ...
            if let Some(extra_fees_msat) = cur_hop
                .candidate
                .htlc_minimum_msat()
                .checked_sub(cur_hop_transferred_amount_msat)
            {
...
fn compute_fees(amount_msat: u64, channel_fees: RoutingFees) -> Option<u64> {
    ...
    if let Some(new_fee) =
        proportional_fee_millions.and_then(|part| (channel_fees.base_msat as u64).checked_add(part / 1_000_000))
    {
        Some(new_fee)
    } else {
    ...
    L::Target: Logger,
{
    ...
    match get_route(
        ...
        route_params.final_cltv_expiry_delta,
        logger,
        scorer,
        random_seed_bytes,
    ) {
    ...
    L::Target: Logger,
{
    ...
    if payment_params.max_total_cltv_expiry_delta <= final_cltv_expiry_delta {
        return Err(LightningError {
            err: "Can't find a route where the maximum total CLTV expiry delta is below the final CLTV expiry."
                .to_owned(),
            action: ErrorAction::IgnoreError,
        });
    }
    ...
    log_trace!(
        logger,
        "Searching for a route from payer {} to payee {} {} MPP and {} first hops {}overriding the network graph",
        our_node_pubkey,
        payment_params.payee_pubkey,
        if allow_mpp { "with" } else { "without" },
        first_hops.map(|hops| hops.len()).unwrap_or(0),
    ...
    let mut first_hop_targets: HashMap<_, Vec<&ChannelDetails>> = HashMap::with_capacity(if first_hops.is_some() {
    ...
    for (_, channels) in first_hop_targets.iter_mut() {
        ...
        channels.sort_unstable_by(|chan_a, chan_b| {
            if chan_b.outbound_capacity_msat < recommended_value_msat
                || chan_a.outbound_capacity_msat < recommended_value_msat
            {
    ...
    log_trace!(
        logger,
        "Building path from {} (payee) to {} (us/payer) for value {} msat.",
        payment_params.payee_pubkey,
        our_node_pubkey,
        final_value_msat
    );
    ...
    'paths_collection: loop {
        ...
        if let Some(first_channels) = first_hop_targets.get(&payee_node_id) {
            for details in first_channels {
                ...
                log_trace!(
                    logger,
                    "{} direct route to payee via SCID {}",
        ...
        for route in payment_params.route_hints.iter().filter(|route| !route.0.is_empty()) {
            ...
            if have_hop_src_in_graph {
                ...
                let prev_hop_iter = core::iter::once(&payment_params.payee_pubkey)
                ...
                for (idx, (hop, prev_hop_id)) in hop_iter.zip(prev_hop_iter).enumerate() {
                    ...
                    aggregate_next_hops_path_penalty_msat =
                        aggregate_next_hops_path_penalty_msat.saturating_add(scorer.channel_penalty_msat(
                    ...
                    if let Some(first_channels) = first_hop_targets.get(&NodeId::from_pubkey(&prev_hop_id)) {
                        for details in first_channels {
                            ...
                            add_entry!(
                                candidate,
                                our_node_id,
                                NodeId::from_pubkey(&prev_hop_id),
                                aggregate_next_hops_fee_msat,
                                path_value_msat,
                                aggregate_next_hops_path_htlc_minimum_msat,
                                aggregate_next_hops_path_penalty_msat,
                    ...
                    let hop_htlc_minimum_msat_inc =
                        if let Some(val) = compute_fees(aggregate_next_hops_path_htlc_minimum_msat, hop.fees) {
                            val
                        } else {
                            break;
                        };
                    let hops_path_htlc_minimum =
                        aggregate_next_hops_path_htlc_minimum_msat.checked_add(hop_htlc_minimum_msat_inc);
                    aggregate_next_hops_path_htlc_minimum_msat = if let Some(val) = hops_path_htlc_minimum {
                        cmp::max(hop_htlc_minimum_msat, val)
                    } else {
                        break;
                    };
            ...
            if node_id == our_node_id {
                ...
                'path_walk: loop {
                    ...
                    if let Some(first_channels) = first_hop_targets.get(&ordered_hops.last().unwrap().0.node_id) {
                        for details in first_channels {
                            if details.short_channel_id.unwrap()
                                == ordered_hops.last().unwrap().0.candidate.short_channel_id()
                            {
                ...
                log_trace!(
                    logger,
                    "Found a path back to us from the target with {} hops contributing up to {} msat: \n {:#?}",
                    ordered_hops.len(),
                    value_contribution_msat,
                    ordered_hops.iter().map(|h| &(h.0)).collect::<Vec<&PathBuildingHop>>()
                );
                ...
                for (payment_hop, _) in payment_path.hops.iter() {
                    let channel_liquidity_available_msat = bookkept_channels_liquidity_available_msat
                ...
                if !prevented_redundant_path_selection {
                    ...
                    let victim_scid = payment_path.hops[(payment_path.hops.len()) / 2]
                        .0
                        .candidate
                        .short_channel_id();
                    log_trace!(
                        logger,
                        "Disabling channel {} for future path building iterations to avoid duplicates.",
                        victim_scid
                    );
                    let victim_liquidity = bookkept_channels_liquidity_available_msat
                        .get_mut(&victim_scid)
                        .unwrap();
                    *victim_liquidity = 0;
                }
        ...
        if already_collected_value_msat >= recommended_value_msat || !found_new_path {
            log_trace!(
                logger,
                "Have now collected {} msat (seeking {} msat) in paths. Last path loop {} a new path.",
                already_collected_value_msat,
                recommended_value_msat,
                if found_new_path { "found" } else { "did not find" }
            );
            break 'paths_collection;
    ...
    for _ in 0..num_permutations {
        ...
        for payment_path in &payment_paths {
            ...
            if aggregate_route_value_msat > final_value_msat {
                ...
                cur_route.sort_unstable_by_key(|path| {
                    path.hops
                        .iter()
    ...
    for payment_path in drawn_routes.first().unwrap() {
        let mut path = payment_path
            .hops
            .iter()
            .map(|(payment_hop, node_features)| {
                Ok(RouteHop {
                    pubkey: PublicKey::from_slice(payment_hop.node_id.as_slice()).map_err(|_| LightningError {
                        err: format!("Public key {:?} is invalid", &payment_hop.node_id),
        ...
        path.iter_mut()
            .rev()
            .fold(final_cltv_expiry_delta, |prev_cltv_expiry_delta, hop| {
...
fn add_random_cltv_offset(
    ...
    for path in route.paths.iter_mut() {
        ...
        let mut nodes_to_avoid: [NodeId; 3] = [
            NodeId::from_pubkey(&path.last().unwrap().pubkey),
        ...
        for random_hop in 0..random_walk_length {
            ...
            if let Some(cur_node_id) = cur_hop {
                if let Some(cur_node) = network_nodes.get(&cur_node_id) {
                    ...
                    {
                        random_channel
                            .as_directed_from(&cur_node_id)
                            .map(|(dir_info, next_id)| {
...
mod tests {
    ...
    use ln::msgs::{
        ChannelAnnouncement, ChannelUpdate, ErrorAction, LightningError, NodeAnnouncement, OptionalField,
        RoutingMessageHandler, UnsignedChannelAnnouncement, UnsignedChannelUpdate, UnsignedNodeAnnouncement,
    };
    use routing::network_graph::{NetGraphMsgHandler, NetworkGraph, NodeId};
    use routing::router::{
        add_random_cltv_offset, get_route, PaymentParameters, Route, RouteHint, RouteHintHop, RouteHop, RoutingFees,
    ...
    fn get_nodes(secp_ctx: &Secp256k1<All>) -> (SecretKey, PublicKey, Vec<SecretKey>, Vec<PublicKey>) {
        let privkeys: Vec<SecretKey> = (2..10)
            .map(|i| SecretKey::from_slice(&hex::decode(format!("{:02x}", i).repeat(32)).unwrap()[..]).unwrap())
            .collect();

        let pubkeys = privkeys
            .iter()
            .map(|secret| PublicKey::from_secret_key(&secp_ctx, secret))
            .collect();
    ...
    fn build_graph() -> (
        ...
        NetGraphMsgHandler<
            ...
            sync::Arc<crate::util::test_utils::TestLogger>,
        >,
        sync::Arc<test_utils::TestChainSource>,
    ...
    fn partial_route_hint_test() {
        ...
        let payment_params =
            PaymentParameters::from_node_id(nodes[6]).with_route_hints(last_hops_multi_private_channels(&nodes));
        let route = get_route(
            &our_id,
            &payment_params,
            &network_graph.read_only(),
    ...
    fn private_multi_hint_last_hops_test() {
        ...
        let non_announced_privkey =
            SecretKey::from_slice(&hex::decode(format!("{:02x}", 0xf0).repeat(32)).unwrap()[..]).unwrap();
    ...
    fn last_hops_with_public_channel_test() {
        ...
        let (_, our_id, _, nodes) = get_nodes(&secp_ctx);
        let payment_params =
            PaymentParameters::from_node_id(nodes[6]).with_route_hints(last_hops_with_public_channel(&nodes));
        let scorer = test_utils::TestScorer::with_penalty(0);
        let keys_manager = test_utils::TestKeysInterface::new(&[0u8; 32], Network::Testnet);
        let random_seed_bytes = keys_manager.get_secure_random_bytes();
    ...
    fn do_unannounced_path_test(
        last_hop_htlc_max: Option<u64>,
        last_hop_fee_prop: u32,
        outbound_capacity_msat: u64,
        route_val: u64,
    ) -> Result<Route, LightningError> {
        let source_node_id = PublicKey::from_secret_key(
            &Secp256k1::new(),
            &SecretKey::from_slice(&hex::decode(format!("{:02}", 41).repeat(32)).unwrap()[..]).unwrap(),
        );
        let middle_node_id = PublicKey::from_secret_key(
            &Secp256k1::new(),
            &SecretKey::from_slice(&hex::decode(format!("{:02}", 42).repeat(32)).unwrap()[..]).unwrap(),
        );
        let target_node_id = PublicKey::from_secret_key(
            &Secp256k1::new(),
            &SecretKey::from_slice(&hex::decode(format!("{:02}", 43).repeat(32)).unwrap()[..]).unwrap(),
        );
        ...
        get_route(
            &source_node_id,
            &payment_params,
            &NetworkGraph::new(genesis_block(Network::Testnet).header.block_hash()).read_only(),
            Some(&our_chans.iter().collect::<Vec<_>>()),
            route_val,
            42,
    ...
    fn available_amount_while_routing_test() {
        ...
        *chain_monitor.utxo_ret.lock().unwrap() = Ok(TxOut {
            value: 15,
    ...
    fn mpp_with_last_hops() {
        ...
        let keys_manager = test_utils::TestKeysInterface::new(&[0u8; 32], Network::Testnet);
        let random_seed_bytes = keys_manager.get_secure_random_bytes();
        let payment_params = PaymentParameters::from_node_id(PublicKey::from_slice(&[02; 33]).unwrap())
    ...
    fn htlc_max_reduction_below_min() {
...

        {
            ...
            assert_eq!(route.paths[0][1].cltv_expiry_delta, 42);
            assert_eq!(
                route.paths[0][1].node_features.le_flags(),
                InvoiceFeatures::known().le_flags()
            );
            assert_eq!(route.paths[0][1].channel_features.le_flags(), &id_to_feature_flags(13));
        }
    }
    ...
    fn avoids_routing_through_bad_channels_and_nodes() {
        ...
        let scorer = BadNodeScorer {
            node_id: NodeId::from_pubkey(&nodes[2]),
        };
        match get_route(
            &our_id,
            &payment_params,
    ...
    fn total_fees_single_path() {
        let route = Route {
            paths: vec![vec![
                ...
                RouteHop {
                    pubkey: PublicKey::from_slice(
                        &hex::decode("0324653eac434488002cc06bbfb7f10fe18991e35f9fe4302dbea6d2353dc0ab1c").unwrap()[..],
                    )
                    .unwrap(),
                    channel_features: ChannelFeatures::empty(),
                    node_features: NodeFeatures::empty(),
                ...
                RouteHop {
                    pubkey: PublicKey::from_slice(
                        &hex::decode("027f31ebc5462c1fdce1b737ecff52d37d75dea43ce11c74d25aa297165faa2007").unwrap()[..],
                    )
                    .unwrap(),
                    channel_features: ChannelFeatures::empty(),
                    node_features: NodeFeatures::empty(),
    ...
    fn limits_total_cltv_delta() {
        ...
        let feasible_payment_params = PaymentParameters::from_node_id(nodes[6])
        ...
        let fail_payment_params = PaymentParameters::from_node_id(nodes[6])
            .with_route_hints(last_hops(&nodes))
    ...
    fn adds_and_limits_cltv_offset() {
        ...
        let cltv_expiry_deltas_default = route_default.paths[0]
            .iter()
        ...
        let cltv_expiry_deltas_limited = route_limited.paths[0]
            .iter()
            .map(|h| h.cltv_expiry_delta)
            .collect::<Vec<u32>>();
    ...
    fn generate_routes() {
        ...
        'load_endpoints: for _ in 0..10 {
            loop {
                ...
                let src =
                    &PublicKey::from_slice(nodes.keys().skip(seed % nodes.len()).next().unwrap().as_slice()).unwrap();
                seed = seed.overflowing_mul(0xdeadbeef).0;
                let dst =
                    PublicKey::from_slice(nodes.keys().skip(seed % nodes.len()).next().unwrap().as_slice()).unwrap();
...
mod benches {
    ...
    fn generate_routes<S: Score>(bench: &mut Bencher, graph: &NetworkGraph, mut scorer: S, features: InvoiceFeatures) {
        let nodes = graph.read_only().nodes().clone();
