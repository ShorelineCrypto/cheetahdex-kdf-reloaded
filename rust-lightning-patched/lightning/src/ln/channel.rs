...
use ln::chan_utils;
use ln::chan_utils::{
    get_commitment_transaction_number_obscure_factor, htlc_success_tx_weight, htlc_timeout_tx_weight,
...
use ln::channelmanager::{
    CounterpartyForwardingInfo, HTLCFailReason, HTLCFailureMsg, HTLCSource, PendingHTLCInfo, PendingHTLCStatus,
    RAACommitmentOrder, BREAKDOWN_TIMEOUT, MAX_LOCAL_BREAKDOWN_TIMEOUT, MIN_CLTV_EXPIRY_DELTA,
};
...
use util::config::{ChannelConfig, ChannelHandshakeLimits, UserConfig};
...
impl<Signer: Sign> Channel<Signer> {
    ...
    pub(crate) fn maybe_handle_error_without_close(&mut self, chain_hash: BlockHash) -> Result<msgs::OpenChannel, ()> {
        if !self.is_outbound() || self.channel_state != ChannelState::OurInitSent as u32 {
            return Err(());
        }
        ...
        F::Target: FeeEstimator,
    {
        ...
        if channel_value_satoshis >= MAX_FUNDING_SATOSHIS {
            return Err(APIError::APIMisuseError {
                err: format!(
                    "funding_value must be smaller than {}, it was {}",
                    MAX_FUNDING_SATOSHIS, channel_value_satoshis
                ),
            });
        }
        ...
        let holder_selected_channel_reserve_satoshis =
            Channel::<Signer>::get_holder_selected_channel_reserve_satoshis(channel_value_satoshis);
        ...
        if value_to_self_msat < commitment_tx_fee {
            return Err(APIError::APIMisuseError {
                err: format!(
                    "Funding amount ({}) can't even pay fee for initial commitment transaction fee of {}.",
                    value_to_self_msat / 1000,
                    commitment_tx_fee / 1000
                ),
            });
        }
        ...
        Ok(Channel {
            ...
            holder_max_htlc_value_in_flight_msat: Self::get_holder_max_htlc_value_in_flight_msat(
                channel_value_satoshis,
            ),
            counterparty_selected_channel_reserve_satoshis: None, // Filled in in accept_channel
            holder_selected_channel_reserve_satoshis,
            counterparty_htlc_minimum_msat: 0,
            holder_htlc_minimum_msat: if config.own_channel_config.our_htlc_minimum_msat == 0 {
                1
            } else {
        ...
        F::Target: FeeEstimator,
    {
        ...
        let upper_limit = cmp::max(
            250 * 25,
            fee_estimator.get_est_sat_per_1000_weight(ConfirmationTarget::HighPriority) as u64 * 10,
        );
        if feerate_per_kw as u64 > upper_limit {
            return Err(ChannelError::Close(format!(
                "Peer's feerate much too high. Actual: {}. Our expected upper limit: {}",
                feerate_per_kw, upper_limit
            )));
        }
        ...
        if feerate_per_kw + 250 < lower_limit {
            return Err(ChannelError::Close(format!(
                "Peer's feerate much too low. Actual: {}. Our expected lower limit: {} (- 250)",
        ...
        L::Target: Logger,
    {
        ...
        if msg.funding_satoshis >= MAX_FUNDING_SATOSHIS {
            return Err(ChannelError::Close(format!(
                "Funding must be smaller than {}. It was {}",
                MAX_FUNDING_SATOSHIS, msg.funding_satoshis
            )));
        }
        if msg.channel_reserve_satoshis > msg.funding_satoshis {
            return Err(ChannelError::Close(format!(
                "Bogus channel_reserve_satoshis ({}). Must be not greater than funding_satoshis: {}",
                msg.channel_reserve_satoshis, msg.funding_satoshis
            )));
        }
        let funding_value = (msg.funding_satoshis - msg.channel_reserve_satoshis) * 1000;
        if msg.push_msat > funding_value {
            return Err(ChannelError::Close(format!(
                "push_msat {} was larger than funding value {}",
                msg.push_msat, funding_value
            )));
        }
        if msg.dust_limit_satoshis > msg.funding_satoshis {
            return Err(ChannelError::Close(format!(
                "dust_limit_satoshis {} was larger than funding_satoshis {}. Peer never wants payout outputs?",
                msg.dust_limit_satoshis, msg.funding_satoshis
            )));
        }
        ...
        let max_counterparty_selected_contest_delay = u16::min(
            config.peer_channel_config_limits.their_to_self_delay,
        ...
        if msg.to_self_delay > max_counterparty_selected_contest_delay {
            return Err(ChannelError::Close(format!(
                ...
                max_counterparty_selected_contest_delay, msg.to_self_delay
            )));
        }
        ...
        if msg.funding_satoshis < config.peer_channel_config_limits.min_funding_satoshis {
            return Err(ChannelError::Close(format!(
                ...
                msg.funding_satoshis, config.peer_channel_config_limits.min_funding_satoshis
            )));
        }
        if msg.htlc_minimum_msat > config.peer_channel_config_limits.max_htlc_minimum_msat {
            return Err(ChannelError::Close(format!(
                ...
                msg.htlc_minimum_msat, config.peer_channel_config_limits.max_htlc_minimum_msat
            )));
        }
        if msg.max_htlc_value_in_flight_msat < config.peer_channel_config_limits.min_max_htlc_value_in_flight_msat {
            return Err(ChannelError::Close(format!(
                ...
                msg.max_htlc_value_in_flight_msat, config.peer_channel_config_limits.min_max_htlc_value_in_flight_msat
            )));
        }
        if msg.channel_reserve_satoshis > config.peer_channel_config_limits.max_channel_reserve_satoshis {
            return Err(ChannelError::Close(format!(
                ...
                msg.channel_reserve_satoshis, config.peer_channel_config_limits.max_channel_reserve_satoshis
            )));
        }
        if msg.max_accepted_htlcs < config.peer_channel_config_limits.min_max_accepted_htlcs {
            return Err(ChannelError::Close(format!(
                "max_accepted_htlcs ({}) is less than the user specified limit ({})",
                msg.max_accepted_htlcs, config.peer_channel_config_limits.min_max_accepted_htlcs
            )));
        }
        ...
        if config.peer_channel_config_limits.force_announced_channel_preference {
            if local_config.announced_channel != announced_channel {
                return Err(ChannelError::Close(
                    "Peer tried to open channel but their announcement preference is different from ours".to_owned(),
                ));
            }
        }
        ...
        let holder_selected_channel_reserve_satoshis =
            Channel::<Signer>::get_holder_selected_channel_reserve_satoshis(msg.funding_satoshis);
        if holder_selected_channel_reserve_satoshis < MIN_CHAN_DUST_LIMIT_SATOSHIS {
            return Err(ChannelError::Close(format!(
        ...
        if holder_selected_channel_reserve_satoshis < msg.dust_limit_satoshis {
            return Err(ChannelError::Close(format!(
                ...
                msg.dust_limit_satoshis, holder_selected_channel_reserve_satoshis
            )));
        }
        ...
        let commitment_tx_fee =
            Self::commit_tx_fee_msat(msg.feerate_per_kw, MIN_AFFORDABLE_HTLC_COUNT, opt_anchors) / 1000;
        if funders_amount_msat / 1000 < commitment_tx_fee {
            return Err(ChannelError::Close(format!(
                "Funding amount ({} sats) can't even pay fee for initial commitment transaction fee of {} sats.",
        ...
        L::Target: Logger,
    {
        ...
        macro_rules! add_htlc_output {
            ($htlc: expr, $outbound: expr, $source: expr, $state_name: expr) => {
                if $outbound == local {
                    ...
                    if $htlc.amount_msat / 1000
                        >= broadcaster_dust_limit_satoshis
                            + (feerate_per_kw as u64 * htlc_timeout_tx_weight(self.opt_anchors()) / 1000)
                    {
                        log_trace!(
                            logger,
                            "   ...including {} {} HTLC {} (hash {}) with value {}",
                        ...
                        log_trace!(
                            logger,
                            "   ...including {} {} dust HTLC {} (hash {}) with value {} due to dust limit",
                            if $outbound { "outbound" } else { "inbound" },
                            $state_name,
                            $htlc.htlc_id,
                            log_bytes!($htlc.payment_hash.0),
                    ...
                    if $htlc.amount_msat / 1000
                        >= broadcaster_dust_limit_satoshis
                            + (feerate_per_kw as u64 * htlc_success_tx_weight(self.opt_anchors()) / 1000)
                    {
                        log_trace!(
                            logger,
                            "   ...including {} {} HTLC {} (hash {}) with value {}",
                        ...
                        log_trace!(
                            logger,
                            "   ...including {} {} dust HTLC {} (hash {}) with value {}",
                            if $outbound { "outbound" } else { "inbound" },
                            $state_name,
                            $htlc.htlc_id,
                            log_bytes!($htlc.payment_hash.0),
        ...
        let mut value_to_self_msat: i64 =
        ...
        let mut value_to_remote_msat: i64 = (self.channel_value_satoshis * 1000) as i64
        ...
        #[cfg(debug_assertions)]
        {
            ...
            debug_assert!(
                broadcaster_max_commitment_tx_output.0 <= value_to_self_msat as u64
                    || value_to_self_msat / 1000 >= self.counterparty_selected_channel_reserve_satoshis.unwrap() as i64
            );
            ...
            debug_assert!(
                broadcaster_max_commitment_tx_output.1 <= value_to_remote_msat as u64
                    || value_to_remote_msat / 1000 >= self.holder_selected_channel_reserve_satoshis as i64
            );
            broadcaster_max_commitment_tx_output.1 =
                cmp::max(broadcaster_max_commitment_tx_output.1, value_to_remote_msat as u64);
        }

        let total_fee_sat = Channel::<Signer>::commit_tx_fee_sat(
            feerate_per_kw,
            included_non_dust_htlcs.len(),
            self.channel_transaction_parameters.opt_anchors.is_some(),
        );
        let anchors_val = if self.channel_transaction_parameters.opt_anchors.is_some() {
        ...
        value_to_self_msat = if (value_to_self_msat * 1000) < broadcaster_dust_limit_satoshis as i64 {
            0
        } else {
            value_to_self_msat
        };
        value_to_remote_msat = if (value_to_remote_msat * 1000) < broadcaster_dust_limit_satoshis as i64 {
            0
        } else {
    ...
    fn build_closing_transaction(
        ...
        let mut value_to_holder: i64 = (self.value_to_self_msat as i64) / 1000
            - if self.is_outbound() {
                total_fee_satoshis as i64
            } else {
                0
            };
        let mut value_to_counterparty: i64 = ((self.channel_value_satoshis * 1000 - self.value_to_self_msat) as i64
            / 1000)
            - if self.is_outbound() {
                0
            } else {
    ...
    fn build_holder_transaction_keys(&self, commitment_number: u64) -> Result<TxCreationKeys, ChannelError> {
        let per_commitment_point = self
            .holder_signer
            .get_per_commitment_point(commitment_number, &self.secp_ctx);
        ...
        Ok(secp_check!(
            ...
            "Local tx keys generation got bogus keys".to_owned()
        ))
    }
    ...
    fn build_remote_transaction_keys(&self) -> Result<TxCreationKeys, ChannelError> {
        ...
        Ok(secp_check!(
            ...
            "Remote tx keys generation got bogus keys".to_owned()
        ))
    }
    ...
    pub fn get_update_fulfill_htlc_and_commit<L: Deref>(
        ...
        payment_preimage: PaymentPreimage,
        logger: &L,
    ) -> Result<UpdateFulfillCommitFetch, (ChannelError, ChannelMonitorUpdate)>
    where
        L::Target: Logger,
    {
        match self.get_update_fulfill_htlc(htlc_id, payment_preimage, logger) {
            UpdateFulfillFetch::NewClaim {
                ...
                Ok(UpdateFulfillCommitFetch::NewClaim {
                    monitor_update,
                    htlc_value_msat,
                    msgs: Some((update_fulfill_htlc, commitment)),
                })
            },
            UpdateFulfillFetch::NewClaim {
                monitor_update,
    ...
    pub fn get_update_fail_htlc<L: Deref>(
        &mut self,
        htlc_id_arg: u64,
        err_packet: msgs::OnionErrorPacket,
        logger: &L,
        ...
        L::Target: Logger,
    {
                    ...
                    &HTLCUpdateAwaitingACK::FailHTLC { htlc_id, .. } => {
                        if htlc_id_arg == htlc_id {
                            ...
                            return Err(ChannelError::Ignore(
                                "Unable to find a pending HTLC which matched the given HTLC ID".to_owned(),
                            ));
                        }
                    },
                    _ => {},
        ...
        log_trace!(
            logger,
            "Failing HTLC ID {} back with a update_fail_htlc message in channel {}.",
            htlc_id_arg,
            log_bytes!(self.channel_id())
        );
        {
    ...
    pub fn accept_channel(
        ...
        if msg.channel_reserve_satoshis > self.channel_value_satoshis {
            return Err(ChannelError::Close(format!(
                ...
                msg.channel_reserve_satoshis, self.channel_value_satoshis
            )));
        }
        if msg.dust_limit_satoshis > self.holder_selected_channel_reserve_satoshis {
            return Err(ChannelError::Close(format!(
                "Dust limit ({}) is bigger than our channel reserve ({})",
                msg.dust_limit_satoshis, self.holder_selected_channel_reserve_satoshis
            )));
        }
        ...
        if msg.to_self_delay > max_delay_acceptable {
            return Err(ChannelError::Close(format!(
                "They wanted our payments to be delayed by a needlessly long period. Upper limit: {}. Actual: {}",
                max_delay_acceptable, msg.to_self_delay
            )));
        }
        if msg.max_accepted_htlcs < 1 {
            return Err(ChannelError::Close(
        ...
        if msg.minimum_depth > peer_limits.max_minimum_depth {
            return Err(ChannelError::Close(format!(
                ...
                peer_limits.max_minimum_depth, msg.minimum_depth
            )));
        }
        if msg.minimum_depth == 0 {
            ...
            // channel.
            return Err(ChannelError::Close(
                "Minimum confirmation depth must be at least 1".to_owned(),
            ));
        }
        ...
        if let Some(ty) = &msg.channel_type {
            if *ty != self.channel_type {
                return Err(ChannelError::Close(
                    "Channel Type in accept_channel didn't match the one sent in open_channel.".to_owned(),
                ));
            }
    ...
    fn funding_created_signature<L: Deref>(
        &mut self,
        sig: &Signature,
        logger: &L,
    ) -> Result<(Txid, CommitmentTransaction, Signature), ChannelError>
    where
        L::Target: Logger,
    {
        ...
        let initial_commitment_tx = self
            .build_commitment_transaction(
                self.cur_holder_commitment_transaction_number,
                &keys,
                true,
                false,
                logger,
            ...
            .tx;
        {
            ...
            secp_check!(
                self.secp_ctx.verify(&sighash, &sig, self.counterparty_funding_pubkey()),
        ...
        let counterparty_signature = self
            .holder_signer
            .sign_counterparty_commitment(&counterparty_initial_commitment_tx, Vec::new(), &self.secp_ctx)
        ...
        L::Target: Logger,
    {
        if self.is_outbound() {
            return Err(ChannelError::Close(
                "Received funding_created for an outbound channel?".to_owned(),
            ));
        }
        ...
        let (counterparty_initial_commitment_txid, initial_commitment_tx, signature) =
            match self.funding_created_signature(&msg.signature, logger) {
                Ok(res) => res,
        ...
        channel_monitor.provide_latest_counterparty_commitment_tx(
            counterparty_initial_commitment_txid,
            Vec::new(),
            self.cur_counterparty_commitment_transaction_number,
            self.counterparty_cur_commitment_point.unwrap(),
            logger,
        ...
        log_info!(
            logger,
            "Generated funding_signed for peer for channel {}",
            log_bytes!(self.channel_id())
        );
    ...
    pub fn funding_signed<L: Deref>(
        ...
        best_block: BestBlock,
        logger: &L,
    ) -> Result<(ChannelMonitor<Signer>, Transaction), ChannelError>
    where
        L::Target: Logger,
    {
        if !self.is_outbound() {
            return Err(ChannelError::Close(
                "Received funding_signed for an inbound channel?".to_owned(),
            ));
        }
        if self.channel_state & !(ChannelState::MonitorUpdateFailed as u32) != ChannelState::FundingCreated as u32 {
            return Err(ChannelError::Close(
                "Received funding_signed in strange state!".to_owned(),
            ));
        }
        if self.commitment_secrets.get_min_seen_secret() != (1 << 48)
            || self.cur_counterparty_commitment_transaction_number != INITIAL_COMMITMENT_NUMBER
            ...
            .tx;
        {
            ...
            if let Err(_) = self.secp_ctx.verify(
                &sighash,
                &msg.signature,
                &self.get_counterparty_pubkeys().funding_pubkey,
            ) {
        ...
        L::Target: Logger,
    {
			                      ...
			                      (ChannelState::FundingSent as u32 | ChannelState::TheirFundingLocked as u32))
        {
            ...
            let expected_point = if self.cur_counterparty_commitment_transaction_number == INITIAL_COMMITMENT_NUMBER - 1
            {
                ...
                Some(PublicKey::from_secret_key(
                    ...
                    &SecretKey::from_slice(
                        &self
                            .commitment_secrets
                            .get_secret(INITIAL_COMMITMENT_NUMBER - 1)
                            .expect("We should have all prev secrets available"),
                    )
            ...
            if expected_point != Some(msg.next_per_commitment_point) {
                return Err(ChannelError::Close(
                    "Peer sent a reconnect funding_locked with a different point".to_owned(),
                ));
            }
            return Ok(None);
        } else {
            return Err(ChannelError::Close(
                "Peer sent a funding_locked at a strange time".to_owned(),
            ));
        }
        ...
        log_info!(
            logger,
            "Received funding_locked from peer for channel {}",
            log_bytes!(self.channel_id())
        );
    ...
    fn get_inbound_pending_htlc_stats(&self, outbound_feerate_update: Option<u32>) -> HTLCStats {
        ...
        let counterparty_dust_limit_timeout_sat = (self.get_dust_buffer_feerate(outbound_feerate_update) as u64
            * htlc_timeout_tx_weight(self.opt_anchors())
            / 1000)
            + self.counterparty_dust_limit_satoshis;
        let holder_dust_limit_success_sat = (self.get_dust_buffer_feerate(outbound_feerate_update) as u64
            * htlc_success_tx_weight(self.opt_anchors())
            / 1000)
            + self.holder_dust_limit_satoshis;
        for ref htlc in self.pending_inbound_htlcs.iter() {
    ...
    fn get_outbound_pending_htlc_stats(&self, outbound_feerate_update: Option<u32>) -> HTLCStats {
        ...
        let counterparty_dust_limit_success_sat = (self.get_dust_buffer_feerate(outbound_feerate_update) as u64
            * htlc_success_tx_weight(self.opt_anchors())
            / 1000)
            + self.counterparty_dust_limit_satoshis;
        let holder_dust_limit_timeout_sat = (self.get_dust_buffer_feerate(outbound_feerate_update) as u64
            * htlc_timeout_tx_weight(self.opt_anchors())
            / 1000)
            + self.holder_dust_limit_satoshis;
        for ref htlc in self.pending_outbound_htlcs.iter() {
    ...
    fn commit_tx_fee_msat(feerate_per_kw: u32, num_htlcs: usize, opt_anchors: bool) -> u64 {
        ...
        (commitment_tx_base_weight(opt_anchors) + num_htlcs as u64 * COMMITMENT_TX_WEIGHT_PER_HTLC)
    ...
    fn next_local_commit_tx_fee_msat(&self, htlc: HTLCCandidate, fee_spike_buffer_htlc: Option<()>) -> u64 {
        ...
        let real_dust_limit_success_sat = (self.feerate_per_kw as u64 * htlc_success_tx_weight(self.opt_anchors())
            / 1000)
            + self.holder_dust_limit_satoshis;
        let real_dust_limit_timeout_sat = (self.feerate_per_kw as u64 * htlc_timeout_tx_weight(self.opt_anchors())
            / 1000)
            + self.holder_dust_limit_satoshis;
    ...
    pub fn update_add_htlc<F, L: Deref>(
        &mut self,
        msg: &msgs::UpdateAddHTLC,
        mut pending_forward_status: PendingHTLCStatus,
        create_pending_htlc_status: F,
        logger: &L,
    ) -> Result<(), ChannelError>
    where
        ...
        L::Target: Logger,
    {
        ...
        let local_sent_shutdown = (self.channel_state
            & (ChannelState::ChannelFunded as u32 | ChannelState::LocalShutdownSent as u32))
        ...
        let remote_sent_shutdown = (self.channel_state
            & (ChannelState::ChannelFunded as u32 | ChannelState::RemoteShutdownSent as u32))
            != (ChannelState::ChannelFunded as u32);
        ...
        if msg.amount_msat == 0 {
            return Err(ChannelError::Close(
                "Remote side tried to send a 0-msat HTLC".to_owned(),
            ));
        }
        if msg.amount_msat < self.holder_htlc_minimum_msat {
            return Err(ChannelError::Close(format!(
                "Remote side tried to send less than our minimum HTLC value. Lower limit: ({}). Actual: ({})",
                self.holder_htlc_minimum_msat, msg.amount_msat
            )));
        }
        ...
        let exposure_dust_limit_timeout_sats =
            (self.get_dust_buffer_feerate(None) as u64 * htlc_timeout_tx_weight(self.opt_anchors()) / 1000)
                + self.counterparty_dust_limit_satoshis;
        if msg.amount_msat / 1000 < exposure_dust_limit_timeout_sats {
            let on_counterparty_tx_dust_htlc_exposure_msat = inbound_stats.on_counterparty_tx_dust_exposure_msat
        ...
        let exposure_dust_limit_success_sats =
            (self.get_dust_buffer_feerate(None) as u64 * htlc_success_tx_weight(self.opt_anchors()) / 1000)
                + self.holder_dust_limit_satoshis;
        if msg.amount_msat / 1000 < exposure_dust_limit_success_sats {
            let on_holder_tx_dust_htlc_exposure_msat = inbound_stats.on_holder_tx_dust_exposure_msat
        ...
        let pending_remote_value_msat = self.channel_value_satoshis * 1000 - pending_value_to_self_msat;
        ...
        let remote_commit_tx_fee_msat = if self.is_outbound() {
            0
        } else {
        ...
        if pending_remote_value_msat - msg.amount_msat - remote_commit_tx_fee_msat
            < self.holder_selected_channel_reserve_satoshis * 1000
        {
            return Err(ChannelError::Close(
        ...
        if !self.is_outbound() {
            ...
            let htlc_candidate = HTLCCandidate::new(msg.amount_msat, HTLCInitiator::RemoteOffered);
            let remote_fee_cost_incl_stuck_buffer_msat =
                2 * self.next_remote_commit_tx_fee_msat(htlc_candidate, Some(()));
            if pending_remote_value_msat - msg.amount_msat - self.holder_selected_channel_reserve_satoshis * 1000
            ...
            if self.value_to_self_msat
                < self.counterparty_selected_channel_reserve_satoshis.unwrap() * 1000 + local_commit_tx_fee_msat
            {
                return Err(ChannelError::Close(
                    "Cannot accept HTLC that would put our balance under counterparty-announced channel reserve value"
                        .to_owned(),
                ));
            }
        }
        if self.next_counterparty_htlc_id != msg.htlc_id {
            return Err(ChannelError::Close(format!(
                ...
                self.next_counterparty_htlc_id
            )));
        }
    ...
    fn mark_outbound_htlc_removed(
        &mut self,
        htlc_id: u64,
        check_preimage: Option<PaymentPreimage>,
        ...
        Err(ChannelError::Close(
            "Remote tried to fulfill/fail an HTLC we couldn't find".to_owned(),
        ))
    }
    ...
    pub fn update_fail_malformed_htlc(
        ...
        if self.channel_state & (ChannelState::PeerDisconnected as u32) == ChannelState::PeerDisconnected as u32 {
            return Err(ChannelError::Close(
                "Peer sent update_fail_malformed_htlc when we needed a channel_reestablish".to_owned(),
            ));
        }
    ...
    pub fn commitment_signed<L: Deref>(
        ...
        (msgs::RevokeAndACK, Option<msgs::CommitmentSigned>, ChannelMonitorUpdate),
        ...
        L::Target: Logger,
    {
        ...
        if self.channel_state & (ChannelState::PeerDisconnected as u32) == ChannelState::PeerDisconnected as u32 {
            return Err((
                None,
                ChannelError::Close("Peer sent commitment_signed when we needed a channel_reestablish".to_owned()),
            ));
        }
            ...
            && self.last_sent_closing_fee.is_some()
        {
            return Err((
                None,
                ChannelError::Close(
                    "Peer sent commitment_signed after we'd started exchanging closing_signeds".to_owned(),
                ),
            ));
        }
        ...
        let keys = self
            .build_holder_transaction_keys(self.cur_holder_commitment_transaction_number)
            .map_err(|e| (None, e))?;

        let commitment_stats = self.build_commitment_transaction(
            self.cur_holder_commitment_transaction_number,
            &keys,
            true,
            false,
        ...
        let commitment_txid = {
            ...
            if let Err(_) = self
                .secp_ctx
                .verify(&sighash, &msg.signature, &self.counterparty_funding_pubkey())
            {
                return Err((
                    None,
                    ChannelError::Close("Invalid commitment tx signature from peer".to_owned()),
                ));
            }
            bitcoin_tx.txid
        };
        ...
        for (idx, (htlc, source)) in htlcs_cloned.drain(..).enumerate() {
            if let Some(_) = htlc.transaction_output_index {
                ...
                let htlc_sighashtype = if self.opt_anchors() {
                    SigHashType::SinglePlusAnyoneCanPay
                } else {
                    SigHashType::All
                };
                let htlc_sighash = hash_to_message!(
                    &bip143::SigHashCache::new(&htlc_tx).signature_hash(
                        0,
                        &htlc_redeemscript,
                ...
                if let Err(_) = self.secp_ctx.verify(
                    ...
                    return Err((
                        None,
                        ChannelError::Close("Invalid HTLC tx signature from peer".to_owned()),
                    ));
                }
        ...
        let next_per_commitment_point = self
            .holder_signer
            .get_per_commitment_point(self.cur_holder_commitment_transaction_number - 1, &self.secp_ctx);
        self.holder_signer
            .validate_holder_commitment(&holder_commitment_tx, commitment_stats.preimages)
            .map_err(|_| {
                (
                    None,
        ...
        let per_commitment_secret = self
            .holder_signer
            .release_commitment_secret(self.cur_holder_commitment_transaction_number + 1);
        ...
        if (self.channel_state & ChannelState::MonitorUpdateFailed as u32) != 0 {
            ...
            return Err((
                Some(monitor_update),
                ChannelError::Ignore("Previous monitor update failure prevented generation of RAA".to_owned()),
            ));
        }

        let commitment_signed = if need_commitment
    ...
    pub fn maybe_free_holding_cell_htlcs<L: Deref>(
        &mut self,
        logger: &L,
    ) -> Result<
        (
            Option<(msgs::CommitmentUpdate, ChannelMonitorUpdate)>,
    ...
    fn free_holding_cell_htlcs<L: Deref>(
        &mut self,
        logger: &L,
    ) -> Result<
        (
        ...
        L::Target: Logger,
    {
        ...
        if self.holding_cell_htlc_updates.len() != 0 || self.holding_cell_update_fee.is_some() {
            log_trace!(
                ...
                self.holding_cell_htlc_updates.len(),
                if self.holding_cell_update_fee.is_some() {
                    " and a fee update"
                } else {
                    ""
                },
                ...
                match &htlc_update {
                            ...
                            Err(e) => {
                                match e {
                                    ChannelError::Ignore(ref msg) => {
                                        log_info!(
                                            logger,
                                            "Failed to send HTLC with payment_hash {} due to {} in channel {}",
                    ...
                    &HTLCUpdateAwaitingACK::ClaimHTLC {
                        ...
                        let (update_fulfill_msg_option, mut additional_monitor_update) =
                            if let UpdateFulfillFetch::NewClaim {
                                msg, monitor_update, ..
        ...
        L::Target: Logger,
    {
        ...
        if let Some(counterparty_prev_commitment_point) = self.counterparty_prev_commitment_point {
            if PublicKey::from_secret_key(&self.secp_ctx, &secret) != counterparty_prev_commitment_point {
                return Err(ChannelError::Close(
                    "Got a revoke commitment secret which didn't correspond to their current pubkey".to_owned(),
                ));
            }
        }
        ...
        self.holder_signer
            .validate_counterparty_revocation(self.cur_counterparty_commitment_transaction_number + 1, &secret)
        ...
        if (self.channel_state & ChannelState::MonitorUpdateFailed as u32) == ChannelState::MonitorUpdateFailed as u32 {
            ...
            self.monitor_pending_finalized_fulfills
                .append(&mut finalized_claimed_htlcs);
            log_debug!(
                logger,
                "Received a valid revoke_and_ack for channel {} but awaiting a monitor update resolution to reply.",
                log_bytes!(self.channel_id())
            );
            return Ok(RAAUpdates {
                commitment_update: None,
                finalized_claimed_htlcs: Vec::new(),
                accepted_htlcs: Vec::new(),
        ...
        match self.free_holding_cell_htlcs(logger)? {
            (Some((mut commitment_update, mut additional_update)), htlcs_to_fail) => {
                ...
                commitment_update
                    ...
                    .reserve(update_fail_malformed_htlcs.len());
            ...
            (None, htlcs_to_fail) => {
                if require_commitment {
                    ...
                    log_debug!(
                        logger,
                        "Received a valid revoke_and_ack for channel {} with no reply necessary.",
                        log_bytes!(self.channel_id())
                    );
                    Ok(RAAUpdates {
                        commitment_update: None,
    ...
    fn send_update_fee<L: Deref>(&mut self, feerate_per_kw: u32, logger: &L) -> Option<msgs::UpdateFee>
    where
        L::Target: Logger,
    {
        if !self.is_outbound() {
        ...
        let keys = if let Ok(keys) = self.build_holder_transaction_keys(self.cur_holder_commitment_transaction_number) {
            keys
        } else {
            return None;
        };
        let commitment_stats =
            self.build_commitment_transaction(self.cur_holder_commitment_transaction_number, &keys, true, true, logger);
        let buffer_fee_msat = Channel::<Signer>::commit_tx_fee_sat(
            feerate_per_kw,
            commitment_stats.num_nondust_htlcs
        ...
        let holder_tx_dust_exposure =
            inbound_stats.on_holder_tx_dust_exposure_msat + outbound_stats.on_holder_tx_dust_exposure_msat;
        let counterparty_tx_dust_exposure =
            inbound_stats.on_counterparty_tx_dust_exposure_msat + outbound_stats.on_counterparty_tx_dust_exposure_msat;
        ...
        if (self.channel_state & (ChannelState::AwaitingRemoteRevoke as u32 | ChannelState::MonitorUpdateFailed as u32))
            != 0
        {
    ...
    pub fn send_update_fee_and_commit<L: Deref>(
        &mut self,
        feerate_per_kw: u32,
        logger: &L,
    ...
    /// completed.
    pub fn remove_uncommitted_htlcs_and_mark_paused<L: Deref>(&mut self, logger: &L)
    where
        L::Target: Logger,
    {
        assert_eq!(self.channel_state & ChannelState::ShutdownComplete as u32, 0);
    ...
    /// to the remote side.
    pub fn monitor_updating_restored<L: Deref>(
        &mut self,
        logger: &L,
        node_pk: PublicKey,
        genesis_block_hash: BlockHash,
        ...
        L::Target: Logger,
    {
        ...
        let funding_broadcastable =
            if self.channel_state & (ChannelState::FundingSent as u32) != 0 && self.is_outbound() {
        ...
        let funding_locked = if self.monitor_pending_funding_locked {
            ...
            let next_per_commitment_point = self
                .holder_signer
                .get_per_commitment_point(self.cur_holder_commitment_transaction_number, &self.secp_ctx);
        ...
        mem::swap(
            ...
            &mut self.monitor_pending_finalized_fulfills,
        );
        ...
        log_debug!(
            logger,
            "Restored monitor updating in channel {} resulting in {}{} commitment update and {} RAA, with {} first",
            log_bytes!(self.channel_id()),
            if funding_broadcastable.is_some() {
                "a funding broadcastable, "
            } else {
                ""
            },
    ...
    fn get_last_revoke_and_ack(&self) -> msgs::RevokeAndACK {
        ...
        let per_commitment_secret = self
            .holder_signer
            .release_commitment_secret(self.cur_holder_commitment_transaction_number + 2);
        msgs::RevokeAndACK {
            channel_id: self.channel_id,
            per_commitment_secret,
            next_per_commitment_point,
    ...
    fn get_last_commitment_update<L: Deref>(&self, logger: &L) -> msgs::CommitmentUpdate
    where
        L::Target: Logger,
    {
        ...
        msgs::CommitmentUpdate {
            ...
            commitment_signed: self
                .send_commitment_no_state_update(logger)
    ...
    pub fn channel_reestablish<L: Deref>(
        &mut self,
        ...
        L::Target: Logger,
    {
        ...
        {
            return Err(ChannelError::Close(
                "Peer sent a garbage channel_reestablish".to_owned(),
            ));
        }
        ...
        if msg.next_remote_commitment_number > 0 {
            match msg.data_loss_protect {
                OptionalField::Present(ref data_loss) => {
                    let expected_point = self.holder_signer.get_per_commitment_point(
                        INITIAL_COMMITMENT_NUMBER - msg.next_remote_commitment_number + 1,
                        &self.secp_ctx,
                    );
                    let given_secret =
                        SecretKey::from_slice(&data_loss.your_last_per_commitment_secret).map_err(|_| {
                            ChannelError::Close(
                                "Peer sent a garbage channel_reestablish with unparseable secret key".to_owned(),
                            )
                        })?;
                    ...
                    if msg.next_remote_commitment_number
                        > INITIAL_COMMITMENT_NUMBER - self.cur_holder_commitment_transaction_number
                    {
        ...
        if self.channel_state & (ChannelState::FundingSent as u32) == ChannelState::FundingSent as u32 {
                ...
                || self.channel_state & (ChannelState::MonitorUpdateFailed as u32) != 0
            {
                if msg.next_remote_commitment_number != 0 {
                    return Err(ChannelError::Close(
                        "Peer claimed they saw a revoke_and_ack but we haven't sent funding_locked yet".to_owned(),
                    ));
                }
            ...
            == INITIAL_COMMITMENT_NUMBER - self.cur_holder_commitment_transaction_number
        {
            ...
            == (INITIAL_COMMITMENT_NUMBER - 1) - self.cur_holder_commitment_transaction_number
        {
            if self.channel_state & (ChannelState::MonitorUpdateFailed as u32) != 0 {
                self.monitor_pending_revoke_and_ack = true;
                None
            ...
            return Err(ChannelError::Close(
                "Peer attempted to reestablish channel with a very old local commitment transaction".to_owned(),
            ));
        };
        ...
        let funding_locked = if msg.next_local_commitment_number == 1
            && INITIAL_COMMITMENT_NUMBER - self.cur_holder_commitment_transaction_number == 1
        {
        ...
        if msg.next_local_commitment_number == next_counterparty_commitment_number {
                ...
                == 0
            {
                ...
                match self.free_holding_cell_htlcs(logger) {
                    ...
                    | Err(ChannelError::CloseDelayBroadcast(_)) => {
                    ...
                    Ok((None, holding_cell_failed_htlcs)) => Ok(ReestablishResponses {
                        funding_locked,
                        shutdown_msg,
                        announcement_sigs,
                        raa: required_revoke,
            ...
            Err(ChannelError::Close(
                "Peer attempted to reestablish channel with a very old remote commitment transaction".to_owned(),
            ))
        }
    }
        ...
        F::Target: FeeEstimator,
    {
        ...
        if let Some(target_feerate) = self.target_closing_feerate_sats_per_kw {
            let min_feerate = if self.is_outbound() {
                target_feerate
            } else {
                cmp::min(self.feerate_per_kw, target_feerate)
            };
        ...
        let tx_weight = self.get_closing_transaction_weight(
            ...
            Some(self.counterparty_shutdown_scriptpubkey.as_ref().unwrap()),
        );
        ...
        let proposed_max_total_fee_satoshis = if self.is_outbound() {
            ...
            cmp::max(
                normal_feerate as u64 * tx_weight / 1000 + self.config.force_close_avoidance_max_fee_satoshis,
        ...
        K::Target: KeysInterface<Signer = Signer>,
    {
        ...
        if !script::is_bolt2_compliant(&msg.scriptpubkey, their_features) {
            return Err(ChannelError::Warn(format!(
                ...
                msg.scriptpubkey.to_bytes().to_hex()
            )));
        }
        ...
        let send_shutdown =
            (self.channel_state & ChannelState::LocalShutdownSent as u32) != ChannelState::LocalShutdownSent as u32;
        ...
        F::Target: FeeEstimator,
    {
        if self.channel_state & BOTH_SIDES_SHUTDOWN_MASK != BOTH_SIDES_SHUTDOWN_MASK {
            return Err(ChannelError::Close(
                "Remote end sent us a closing_signed before both sides provided a shutdown".to_owned(),
            ));
        }
        if self.channel_state & (ChannelState::PeerDisconnected as u32) == ChannelState::PeerDisconnected as u32 {
            return Err(ChannelError::Close(
        ...
        if !self.pending_inbound_htlcs.is_empty() || !self.pending_outbound_htlcs.is_empty() {
            return Err(ChannelError::Close(
                "Remote end sent us a closing_signed while there were still pending HTLCs".to_owned(),
            ));
        }
        if msg.fee_satoshis > 21_000_000 * 1_0000_0000 {
        ...
        if self.is_outbound() && self.last_sent_closing_fee.is_none() {
            return Err(ChannelError::Close(
                "Remote tried to send a closing_signed when we were supposed to propose the first one".to_owned(),
            ));
        }
        ...
        let sighash = closing_tx
            .trust()
            .get_sighash_all(&funding_redeemscript, self.channel_value_satoshis);

        match self.secp_ctx.verify(
            &sighash,
            &msg.signature,
            &self.get_counterparty_pubkeys().funding_pubkey,
        ) {
        ...
        for outp in closing_tx.trust().built_transaction().output.iter() {
            if !outp.script_pubkey.is_witness_program() && outp.value < MAX_STD_OUTPUT_DUST_LIMIT_SATOSHIS {
                return Err(ChannelError::Close(
                    "Remote sent us a closing_signed with a dust output. Always use segwit closing scripts!".to_owned(),
                ));
            }
        }
        ...
        macro_rules! propose_fee {
            ($new_fee: expr) => {
                ...
                let sig = self
                    ...
                    .sign_closing_transaction(&closing_tx, &self.secp_ctx)
                    .map_err(|_| {
                        ChannelError::Close("External signer refused to sign closing transaction".to_owned())
                    })?;
    ...
    fn check_get_funding_locked(&mut self, height: u32) -> Option<msgs::FundingLocked> {
        ...
        let need_commitment_update = if non_shutdown_state == ChannelState::FundingSent as u32 {
            ...
            panic!(
                "Started confirming a channel in a state pre-FundingSent?: {}",
                self.channel_state
            );
        } else {
            // We got a reorg but not enough to trigger a force close, just ignore.
        ...
        if need_commitment_update {
            if self.channel_state & (ChannelState::MonitorUpdateFailed as u32) == 0 {
                if self.channel_state & (ChannelState::PeerDisconnected as u32) == 0 {
                    let next_per_commitment_point = self
                        .holder_signer
                        .get_per_commitment_point(INITIAL_COMMITMENT_NUMBER - 1, &self.secp_ctx);
    ...
    pub fn transactions_confirmed<L: Deref>(
        &mut self,
        ...
        node_pk: PublicKey,
        logger: &L,
    ) -> Result<(Option<msgs::FundingLocked>, Option<msgs::AnnouncementSignatures>), ClosureReason>
    where
        L::Target: Logger,
    {
        let non_shutdown_state = self.channel_state & (!MULTI_STATE_FLAGS);
            ...
            for &(index_in_block, tx) in txdata.iter() {
                ...
                if non_shutdown_state & !(ChannelState::TheirFundingLocked as u32) == ChannelState::FundingSent as u32 {
                    ...
                    if let Some(funding_locked) = self.check_get_funding_locked(height) {
                        log_info!(
                            logger,
                            "Sending a funding_locked to our peer for channel {}",
                            log_bytes!(self.channel_id)
                        );
    ...
    /// back.
    pub fn best_block_updated<L: Deref>(
        &mut self,
        height: u32,
        highest_header_time: u32,
        genesis_block_hash: BlockHash,
        node_pk: PublicKey,
        logger: &L,
    ) -> Result<
    ...
    fn do_best_block_updated<L: Deref>(
        &mut self,
        height: u32,
        highest_header_time: u32,
        genesis_node_pk: Option<(BlockHash, PublicKey)>,
        logger: &L,
    ) -> Result<
        (
            Option<msgs::FundingLocked>,
            ...
            || (non_shutdown_state & ChannelState::OurFundingLocked as u32) == ChannelState::OurFundingLocked as u32
        {
            ...
            if funding_tx_confirmations < self.minimum_depth.unwrap() as i64 / 2 {
                let err_reason = format!(
                    ...
                    self.minimum_depth.unwrap(),
                    funding_tx_confirmations
                );
                return Err(ClosureReason::ProcessingError { err: err_reason });
            }
        } else if !self.is_outbound()
            && self.funding_tx_confirmed_in.is_none()
    ...
    pub fn funding_transaction_unconfirmed<L: Deref>(&mut self, logger: &L) -> Result<(), ClosureReason>
    where
        L::Target: Logger,
    {
        if self.funding_tx_confirmation_height != 0 {
            ...
            match self.do_best_block_updated(reorg_height, best_time, None, logger) {
                Ok((funding_locked, timed_out_htlcs, announcement_sigs)) => {
                    assert!(
                        funding_locked.is_none(),
                        "We can't generate a funding with 0 confirmations?"
                    );
                    assert!(
                        timed_out_htlcs.is_empty(),
                        "We can't have accepted HTLCs with a timeout before our funding confirmation?"
                    );
                    assert!(
    ...
    /// If an Err is returned, it is a ChannelError::Close (for get_outbound_funding_created)
    fn get_outbound_funding_created_signature<L: Deref>(&mut self, logger: &L) -> Result<Signature, ChannelError>
    where
        L::Target: Logger,
    {
        let counterparty_keys = self.build_remote_transaction_keys()?;
    ...
    pub fn get_outbound_funding_created<L: Deref>(
        &mut self,
    ...
    fn get_channel_announcement(
        ...
        if !self.is_usable() {
            return Err(ChannelError::Ignore(
                "Cannot get a ChannelAnnouncement if the channel is not currently usable".to_owned(),
            ));
        }
        ...
        let msg = msgs::UnsignedChannelAnnouncement {
            ...
            node_id_1: if were_node_one {
                node_id
            } else {
                self.get_counterparty_node_id()
            },
            node_id_2: if were_node_one {
                self.get_counterparty_node_id()
            } else {
                node_id
            },
            bitcoin_key_1: if were_node_one {
                self.get_holder_pubkeys().funding_pubkey
            } else {
                self.counterparty_funding_pubkey().clone()
            },
            bitcoin_key_2: if were_node_one {
                self.counterparty_funding_pubkey().clone()
            } else {
                self.get_holder_pubkeys().funding_pubkey
            },
    ...
    fn get_announcement_sigs<L: Deref>(
        &mut self,
        node_pk: PublicKey,
        genesis_block_hash: BlockHash,
        best_block_height: u32,
        ...
        L::Target: Logger,
    {
        ...
        let announcement = match self.get_channel_announcement(node_pk, genesis_block_hash) {
            ...
            Err(_) => {
                log_trace!(
                    logger,
                    "Cannot create an announcement_signatures as channel is not public."
                );
                return None;
            },
        };
        let (our_node_sig, our_bitcoin_sig) = match self
            .holder_signer
            .sign_channel_announcement(&announcement, &self.secp_ctx)
        {
            Err(_) => {
                log_error!(
                    logger,
                    "Signer rejected channel_announcement signing. Channel will not be announced!"
                );
                return None;
            },
            Ok(v) => v,
    ...
    fn sign_channel_announcement(
        ...
        announcement: msgs::UnsignedChannelAnnouncement,
    ) -> Result<msgs::ChannelAnnouncement, ChannelError> {
        if let Some((their_node_sig, their_bitcoin_sig)) = self.announcement_sigs {
            let were_node_one = announcement.node_id_1 == our_node_id;

            let (our_node_sig, our_bitcoin_sig) = self
                .holder_signer
                .sign_channel_announcement(&announcement, &self.secp_ctx)
            ...
            Err(ChannelError::Ignore(
                "Attempted to sign channel announcement before we'd received announcement_signatures".to_string(),
            ))
        }
    }
    ...
    pub fn announcement_signatures(
        &mut self,
        our_node_id: PublicKey,
        chain_hash: BlockHash,
        best_block_height: u32,
        ...
        if self
            .secp_ctx
            .verify(&msghash, &msg.node_signature, &self.get_counterparty_node_id())
            .is_err()
        {
            return Err(ChannelError::Close(format!(
        ...
        if self
            .secp_ctx
            .verify(&msghash, &msg.bitcoin_signature, self.counterparty_funding_pubkey())
            .is_err()
        {
            return Err(ChannelError::Close(format!(
    ...
    pub fn get_signed_channel_announcement(
        &self,
        our_node_id: PublicKey,
        chain_hash: BlockHash,
        best_block_height: u32,
    ...
    pub fn get_channel_reestablish<L: Deref>(&self, logger: &L) -> msgs::ChannelReestablish
    where
        L::Target: Logger,
    {
        assert_eq!(
        ...
        let data_loss_protect = if self.cur_counterparty_commitment_transaction_number + 1 < INITIAL_COMMITMENT_NUMBER {
            let remote_last_secret = self
                .commitment_secrets
                .get_secret(self.cur_counterparty_commitment_transaction_number + 2)
                .unwrap();
            log_trace!(
                logger,
                "Enough info to generate a Data Loss Protect with per_commitment_secret {} for channel {}",
            ...
            log_info!(
                logger,
                "Sending a data_loss_protect with no previous remote per_commitment_secret for channel {}",
                log_bytes!(self.channel_id())
            );
            OptionalField::Present(DataLossProtect {
        ...
        L::Target: Logger,
    {
        ...
        if amount_msat < self.counterparty_htlc_minimum_msat {
            return Err(ChannelError::Ignore(format!(
                "Cannot send less than their minimum HTLC value ({})",
                self.counterparty_htlc_minimum_msat
            )));
        }
        ...
        if outbound_stats.pending_htlcs + 1 > self.counterparty_max_accepted_htlcs as u32 {
            return Err(ChannelError::Ignore(format!(
                "Cannot push more than their max accepted HTLCs ({})",
                self.counterparty_max_accepted_htlcs
            )));
        }
        ...
        if !self.is_outbound() {
            ...
            if commitment_stats.remote_balance_msat
                < counterparty_commit_tx_fee_msat + holder_selected_chan_reserve_msat
            {
        ...
        if amount_msat / 1000 < exposure_dust_limit_success_sats {
            let on_counterparty_dust_htlc_exposure_msat = inbound_stats.on_counterparty_tx_dust_exposure_msat
                + outbound_stats.on_counterparty_tx_dust_exposure_msat
                + amount_msat;
        ...
        if holder_balance_msat < amount_msat {
            return Err(ChannelError::Ignore(format!(
                "Cannot send value that would overdraw remaining funds. Amount: {}, pending value to self {}",
                amount_msat, holder_balance_msat
            )));
        }
        ...
        if holder_balance_msat - amount_msat - commit_tx_fee_msat < chan_reserve_msat {
            return Err(ChannelError::Ignore(format!(
                "Cannot send value that would put our balance under counterparty-announced channel reserve value ({})",
                chan_reserve_msat
            )));
        }
    ...
    pub fn send_commitment<L: Deref>(
        &mut self,
        logger: &L,
    ...
    fn send_commitment_no_status_check<L: Deref>(
        &mut self,
        logger: &L,
    ) -> Result<(msgs::CommitmentSigned, ChannelMonitorUpdate), ChannelError>
    where
    ...
    fn send_commitment_no_state_update<L: Deref>(
        ...
        (
            ...
            (Txid, Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>),
        ),
        ChannelError,
    >
    where
        L::Target: Logger,
    {
                ...
                *self.next_local_commitment_tx_fee_info_cached.lock().unwrap() = None;
                if let Some(info) = projected_commit_tx_info {
                    ...
                    if info.total_pending_htlcs == total_pending_htlcs
                        ...
                        && info.feerate == self.feerate_per_kw
                    {
                        let actual_fee = Self::commit_tx_fee_msat(
                            self.feerate_per_kw,
                            commitment_stats.num_nondust_htlcs,
                            self.opt_anchors(),
                        );
...

        {
            ...
            let res = self
                .holder_signer
                .sign_counterparty_commitment(&commitment_stats.tx, commitment_stats.preimages, &self.secp_ctx)
                .map_err(|_| ChannelError::Close("Failed to get signatures for new commitment_signed".to_owned()))?;
            signature = res.0;
            ...
            log_trace!(
                logger,
                "Signed remote commitment tx {} (txid {}) with redeemscript {} -> {} in channel {}",
            ...
            for (ref htlc_sig, ref htlc) in htlc_signatures.iter().zip(htlcs) {
                log_trace!(
                    logger,
                    "Signed remote HTLC tx {} with redeemscript {} with pubkey {} -> {} in channel {}",
                    encode::serialize_hex(&chan_utils::build_htlc_transaction(
                        ...
                        htlc,
                        self.opt_anchors(),
                        &counterparty_keys.broadcaster_delayed_payment_key,
                        &counterparty_keys.revocation_key
                    )),
                    encode::serialize_hex(&chan_utils::get_htlc_redeemscript(
                        &htlc,
                        self.opt_anchors(),
                        &counterparty_keys
                    )),
    ...
    pub fn channel_update(&mut self, msg: &msgs::ChannelUpdate) -> Result<(), ChannelError> {
        if msg.contents.htlc_minimum_msat >= self.channel_value_satoshis * 1000 {
            return Err(ChannelError::Close(
                "Minimum htlc value is greater than channel value".to_string(),
            ));
        }
        ...
        K::Target: KeysInterface<Signer = Signer>,
    {
        for htlc in self.pending_outbound_htlcs.iter() {
            if let OutboundHTLCState::LocalAnnounced(_) = htlc.state {
                return Err(APIError::APIMisuseError {
                    err: "Cannot begin shutdown with pending HTLCs. Process pending events first".to_owned(),
                });
            }
        }
        ...
        let monitor_update = if let Some(funding_txo) = self.get_funding_txo() {
            ...
            if self.channel_state
                & (ChannelState::FundingSent as u32
                    | ChannelState::ChannelFunded as u32
                    | ChannelState::ShutdownComplete as u32)
                != 0
            {
                self.latest_monitor_update_id += 1;
                Some((
...
impl<Signer: Sign> Writeable for Channel<Signer> {
    fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
        ...
        let serialized_holder_selected_reserve = if self.holder_selected_channel_reserve_satoshis
            != Self::get_holder_selected_channel_reserve_satoshis(self.channel_value_satoshis)
        {
            Some(self.holder_selected_channel_reserve_satoshis)
        } else {
            None
        };
        let serialized_holder_htlc_max_in_flight = if self.holder_max_htlc_value_in_flight_msat
            != Self::get_holder_max_htlc_value_in_flight_msat(self.channel_value_satoshis)
        {
    ...
    K::Target: KeysInterface<Signer = Signer>,
{
    fn read<R: io::Read>(reader: &mut R, args: (&'a K, u32)) -> Result<Self, DecodeError> {
        ...
        let mut holder_selected_channel_reserve_satoshis = Some(Self::get_holder_selected_channel_reserve_satoshis(
            channel_value_satoshis,
        ));
        let mut holder_max_htlc_value_in_flight_msat =
            Some(Self::get_holder_max_htlc_value_in_flight_msat(channel_value_satoshis));
            ...
            for htlc in pending_outbound_htlcs.iter_mut() {
                match &htlc.state {
                    OutboundHTLCState::AwaitingRemoteRevokeToRemove(OutboundHTLCOutcome::Success(None)) => {
                        htlc.state = OutboundHTLCState::AwaitingRemoteRevokeToRemove(OutboundHTLCOutcome::Success(
                            iter.next().ok_or(DecodeError::InvalidValue)?,
                        ));
                    },
                    OutboundHTLCState::AwaitingRemovedRemoteRevoke(OutboundHTLCOutcome::Success(None)) => {
                        htlc.state = OutboundHTLCState::AwaitingRemovedRemoteRevoke(OutboundHTLCOutcome::Success(
                            iter.next().ok_or(DecodeError::InvalidValue)?,
                        ));
                    },
                    _ => {},
...
mod tests {
    ...
    use bitcoin::secp256k1::{Secp256k1, Signature};
    use chain::chaininterface::{ConfirmationTarget, FeeEstimator};
    use chain::keysinterface::{InMemorySigner, KeyMaterial, KeysInterface, Recipient};
    ...
    use ln::channel::{
        Channel, HTLCCandidate, HTLCInitiator, InboundHTLCOutput, InboundHTLCState, OutboundHTLCOutput,
        OutboundHTLCState,
    };
    ...
    fn test_no_fee_check_overflow() {
        ...
        assert!(
            Channel::<InMemorySigner>::check_remote_fee(&&TestFeeEstimator { fee_est: 42 }, u32::max_value()).is_err()
        );
    }
    ...
    impl KeysInterface for Keys {
        ...
        fn get_destination_script(&self) -> Script {
            ...
            .unwrap();
            let channel_monitor_claim_key_hash =
        ...
        fn read_chan_signer(&self, _data: &[u8]) -> Result<Self::Signer, DecodeError> {
            panic!();
        }
        fn sign_invoice(
            &self,
    ...
    fn upfront_shutdown_script_incompatibility() {
        ...
        match Channel::<EnforcingSigner>::new_outbound(
            &&fee_estimator,
            &&keys_provider,
            node_id,
            &features,
    ...
    fn test_open_channel_msg_fee() {
        ...
        let config = UserConfig::default();
        let node_a_chan = Channel::<EnforcingSigner>::new_outbound(
            &&fee_est,
            &&keys_provider,
            node_a_node_id,
            &InitFeatures::known(),
    ...
    fn test_holder_vs_counterparty_dust_limit() {
        ...
        let config = UserConfig::default();
        let mut node_a_chan = Channel::<EnforcingSigner>::new_outbound(
            &&feeest,
            &&keys_provider,
            node_b_node_id,
            &InitFeatures::known(),
        ...
        let mut node_b_chan = Channel::<EnforcingSigner>::new_from_req(
            &&feeest,
            &&keys_provider,
            node_b_node_id,
            &InitFeatures::known(),
        ...
        node_a_chan
            .accept_channel(
                &accept_channel_msg,
                &config.peer_channel_config_limits,
                &InitFeatures::known(),
            )
            .unwrap();
        ...
        let local_commit_fee_0_htlcs =
            Channel::<EnforcingSigner>::commit_tx_fee_msat(node_a_chan.feerate_per_kw, 0, node_a_chan.opt_anchors());
        assert_eq!(local_commit_tx_fee, local_commit_fee_0_htlcs);

        // Finally, make sure that when Node A calculates the remote's commitment transaction fees, all
        // of the HTLCs are seen to be above the dust limit.
        node_a_chan.channel_transaction_parameters.is_outbound_from_holder = false;
        let remote_commit_fee_3_htlcs =
            Channel::<EnforcingSigner>::commit_tx_fee_msat(node_a_chan.feerate_per_kw, 3, node_a_chan.opt_anchors());
        let htlc_candidate = HTLCCandidate::new(htlc_amount_msat, HTLCInitiator::LocalOffered);
    ...
    fn test_timeout_vs_success_htlc_dust_limit() {
        ...
        let node_id = PublicKey::from_secret_key(&secp_ctx, &SecretKey::from_slice(&[42; 32]).unwrap());
        let config = UserConfig::default();
        let mut chan = Channel::<EnforcingSigner>::new_outbound(
            &&fee_est,
            &&keys_provider,
            node_id,
            &InitFeatures::known(),
        ...
        let commitment_tx_fee_0_htlcs =
            Channel::<EnforcingSigner>::commit_tx_fee_msat(chan.feerate_per_kw, 0, chan.opt_anchors());
        let commitment_tx_fee_1_htlc =
            Channel::<EnforcingSigner>::commit_tx_fee_msat(chan.feerate_per_kw, 1, chan.opt_anchors());

        // If HTLC_SUCCESS_TX_WEIGHT and HTLC_TIMEOUT_TX_WEIGHT were swapped: then this HTLC would be
        // counted as dust when it shouldn't be.
        let htlc_amt_above_timeout =
            ((253 * htlc_timeout_tx_weight(chan.opt_anchors()) / 1000) + chan.holder_dust_limit_satoshis + 1) * 1000;
        let htlc_candidate = HTLCCandidate::new(htlc_amt_above_timeout, HTLCInitiator::LocalOffered);
        let commitment_tx_fee = chan.next_local_commit_tx_fee_msat(htlc_candidate, None);
        assert_eq!(commitment_tx_fee, commitment_tx_fee_1_htlc);

        // If swapped: this HTLC would be counted as non-dust when it shouldn't be.
        let dust_htlc_amt_below_success =
            ((253 * htlc_success_tx_weight(chan.opt_anchors()) / 1000) + chan.holder_dust_limit_satoshis - 1) * 1000;
        ...
        // If swapped: this HTLC would be counted as non-dust when it shouldn't be.
        let dust_htlc_amt_above_timeout =
            ((253 * htlc_timeout_tx_weight(chan.opt_anchors()) / 1000) + chan.counterparty_dust_limit_satoshis + 1)
                * 1000;
        ...
        let htlc_amt_below_success =
            ((253 * htlc_success_tx_weight(chan.opt_anchors()) / 1000) + chan.counterparty_dust_limit_satoshis - 1)
                * 1000;
    ...
    fn channel_reestablish_no_updates() {
        ...
        let funding_created_msg = node_a_chan
            .get_outbound_funding_created(tx.clone(), funding_outpoint, &&logger)
            .unwrap();
        let (funding_signed_msg, _) = node_b_chan
            .funding_created(&funding_created_msg, best_block, &&logger)
            .unwrap();
    ...
    fn outbound_commitment_test() {
        ...
        use ln::chan_utils::{
            ChannelPublicKeys, CounterpartyChannelTransactionParameters, HolderCommitmentTransaction,
        };
        ...
        let mut signer = InMemorySigner::new(
            &secp_ctx,
            SecretKey::from_slice(
                &hex::decode("4242424242424242424242424242424242424242424242424242424242424242").unwrap()[..],
            )
            .unwrap(),
            SecretKey::from_slice(
        ...
        let mut chan = Channel::<InMemorySigner>::new_outbound(
            &&feeest,
            &&keys_provider,
            counterparty_node_id,
            &InitFeatures::known(),
        ...
        chan.channel_transaction_parameters.counterparty_parameters = Some(CounterpartyChannelTransactionParameters {
        ...
        assert_eq!(
            counterparty_pubkeys.htlc_basepoint.serialize()[..],
            hex::decode("032c0b7cf95324a07d05398b240174dc0c2be444d96b159aa6c7f7b1e668680991").unwrap()[..]
        );
        ...
        chan.pending_outbound_htlcs.push({
            ...
            out.payment_hash.0 =
                Sha256::hash(&hex::decode("0505050505050505050505050505050505050505050505050505050505050505").unwrap())
                    .into_inner();
            out
        });
        chan.pending_outbound_htlcs.push({
