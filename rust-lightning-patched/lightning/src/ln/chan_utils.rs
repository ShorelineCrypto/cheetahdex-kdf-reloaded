...
use io;
use ln::chan_utils;
use ln::channel::{ANCHOR_OUTPUT_VALUE_SATOSHI, INITIAL_COMMITMENT_NUMBER};
use prelude::*;
...
pub fn build_htlc_transaction(
    ...
    let mut txouts: Vec<TxOut> = Vec::new();
    txouts.push(TxOut {
        script_pubkey: get_revokeable_redeemscript(revocation_key, contest_delay, broadcaster_delayed_payment_key)
            .to_v0_p2wsh(),
...
impl BuiltCommitmentTransaction {
    ...
    pub fn get_sighash_all(&self, funding_redeemscript: &Script, channel_value_satoshis: u64) -> Message {
        let sighash = &bip143::SigHashCache::new(&self.transaction).signature_hash(
            0,
...
impl<'a> TrustedClosingTransaction<'a> {
    ...
    /// This can be used to verify a signature.
    pub fn get_sighash_all(&self, funding_redeemscript: &Script, channel_value_satoshis: u64) -> Message {
        let sighash = &bip143::SigHashCache::new(&self.inner.built).signature_hash(
            0,
            funding_redeemscript,
            channel_value_satoshis,
            SigHashType::All,
...
impl<'a> TrustedCommitmentTransaction<'a> {
    ...
    pub fn get_htlc_sigs<T: secp256k1::Signing>(
        &self,
        ...
        let holder_htlc_key =
            derive_private_key(secp_ctx, &inner.keys.per_commitment_point, htlc_base_key).map_err(|_| ())?;
    ...
    pub(crate) fn get_signed_htlc_tx(
        ...
        let sighashtype = if self.opt_anchors() {
        ...
        htlc_tx.input[0]
            .witness
            .push(counterparty_signature.serialize_der().to_vec());
...
mod tests {
    ...
    use ln::chan_utils::{
        get_htlc_redeemscript, get_p2wpkh_redeemscript, get_to_countersignatory_with_anchors_redeemscript,
    ...
    fn test_anchors() {
        ...
        assert_eq!(
            ...
            get_p2wpkh_redeemscript(&counterparty_pubkeys.payment_point)
        );
        ...
        assert_eq!(
            ...
            get_to_countersignatory_with_anchors_redeemscript(&counterparty_pubkeys.payment_point).to_v0_p2wsh()
        );
        ...
        assert_eq!(
            ...
            get_htlc_redeemscript(&received_htlc, false, &keys).to_v0_p2wsh()
        );
        assert_eq!(
            tx.built.transaction.output[1].script_pubkey,
            get_htlc_redeemscript(&offered_htlc, false, &keys).to_v0_p2wsh()
        );
        assert_eq!(
            get_htlc_redeemscript(&received_htlc, false, &keys)
                .to_v0_p2wsh()
                .to_hex(),
            "002085cf52e41ba7c099a39df504e7b61f6de122971ceb53b06731876eaeb85e8dc5"
        );
        assert_eq!(
            get_htlc_redeemscript(&offered_htlc, false, &keys)
                .to_v0_p2wsh()
                .to_hex(),
        ...
        assert_eq!(
            ...
            get_htlc_redeemscript(&received_htlc, true, &keys).to_v0_p2wsh()
        );
        assert_eq!(
            tx.built.transaction.output[3].script_pubkey,
            get_htlc_redeemscript(&offered_htlc, true, &keys).to_v0_p2wsh()
        );
        assert_eq!(
            get_htlc_redeemscript(&received_htlc, true, &keys)
                .to_v0_p2wsh()
                .to_hex(),
            "002067114123af3f95405bae4fd930fc95de03e3c86baaee8b2dd29b43dd26cf613c"
        );
        assert_eq!(
            get_htlc_redeemscript(&offered_htlc, true, &keys).to_v0_p2wsh().to_hex(),
