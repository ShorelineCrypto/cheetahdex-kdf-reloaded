...
use chain::channelmonitor::{
    ...
    CLOSED_CHANNEL_UPDATE_ID, CLTV_CLAIM_BUFFER, HTLC_FAIL_BACK_BUFFER, LATENCY_GRACE_PERIOD_BLOCKS,
};
...
use util::events::{ClosureReason, EventHandler, EventsProvider, MessageSendEvent, MessageSendEventsProvider};
use util::logger::{Level, Logger};
use util::scid_utils::fake_scid;
use util::ser::{BigSize, FixedLengthReader, MaybeReadable, Readable, ReadableArgs, VecWriter, Writeable, Writer};
...
mod inbound_payment {
    ...
    pub(super) fn get_payment_preimage(
        ...
        match Method::from_bits((metadata_bytes[0] & 0b1110_0000) >> METHOD_TYPE_OFFSET) {
            Ok(Method::LdkPaymentHash) => derive_ldk_payment_preimage(payment_hash, &iv_bytes, &metadata_bytes, keys)
...
impl PendingOutboundPayment {
    ...
    fn mark_abandoned(&mut self) -> Result<(), ()> {
        ...
        core::mem::swap(
            &mut session_privs,
            match self {
                PendingOutboundPayment::Legacy { .. } | PendingOutboundPayment::Fulfilled { .. } => return Err(()),
...
impl<'a> PersistenceNotifierGuard<'a, fn() -> NotifyOption> {
    ...
    fn notify_on_drop(
        lock: &'a RwLock<()>,
        notifier: &'a PersistenceNotifier,
            ...
            Err(MsgHandleErrInternal {
                ...
                if let Some((shutdown_res, update_option)) = shutdown_finish {
                    ...
                    if let Some(update) = update_option {
                        msg_events.push(events::MessageSendEvent::BroadcastChannelUpdate { msg: update });
                    }
    ...
    ($self: expr, $short_to_id: expr, $channel: expr) => {
        if let Some(short_id) = $channel.get_short_channel_id() {
            ...
            let alias_removed = $self
                ...
                .lock()
                .unwrap()
                .remove(&$channel.outbound_scid_alias());
    ...
    ($self: ident, $err: expr, $short_to_id: expr, $channel: expr, $channel_id: expr) => {
        match $err {
            ChannelError::Warn(msg) => (
                false,
                MsgHandleErrInternal::from_chan_no_close(ChannelError::Warn(msg), $channel_id.clone()),
            ),
            ChannelError::Ignore(msg) => (
                false,
                MsgHandleErrInternal::from_chan_no_close(ChannelError::Ignore(msg), $channel_id.clone()),
            ),
            ...
            ChannelError::CloseDelayBroadcast(msg) => {
                log_error!(
                    $self.logger,
                    "Channel {} need to be shutdown but closing transactions not broadcast due to {}",
                    log_bytes!($channel_id[..]),
                    msg
                );
                update_maps_on_chan_removal!($self, $short_to_id, $channel);
...
macro_rules! break_chan_entry {
    ($self: ident, $res: expr, $channel_state: expr, $entry: expr) => {
        match $res {
            ...
            Err(e) => {
                let (drop, res) =
                    convert_chan_err!($self, e, $channel_state.short_to_id, $entry.get_mut(), $entry.key());
                if drop {
                    $entry.remove_entry();
                }
                break Err(res);
	 ...
	 $pending_forwards: expr, $funding_broadcastable: expr, $funding_locked: expr, $announcement_sigs: expr) => {{
        ...
        let res = loop {
            ...
            if !forwards.is_empty() {
                htlc_forwards = Some((
                    $channel_entry
                        ...
                        .expect("We can't have pending forwards before funding confirmation"),
                    $channel_entry.get().get_funding_txo().unwrap(),
                    forwards,
                ));
            }
            ...
            if let Some(monitor_update) = chanmon_update {
                ...
                if let Err(e) = $self
                    .chain_monitor
                    .update_channel($channel_entry.get().get_funding_txo().unwrap(), monitor_update)
                {
            ...
            macro_rules! handle_cs {
                () => {
                    if let Some(update) = $commitment_update {
                        $channel_state
                            .pending_msg_events
                            .push(events::MessageSendEvent::UpdateHTLCs {
                                node_id: counterparty_node_id,
                                updates: update,
                            });
                    }
            ...
            macro_rules! handle_raa {
                () => {
                    if let Some(revoke_and_ack) = $raa {
                        $channel_state
                            .pending_msg_events
                            .push(events::MessageSendEvent::SendRevokeAndACK {
                                node_id: counterparty_node_id,
                                msg: revoke_and_ack,
                            });
                    }
    ...
    L::Target: Logger,
{
    ...
    pub fn create_channel(
        ...
        let _persistence_guard =
            PersistenceNotifierGuard::notify_on_drop(&self.total_consistency_lock, &self.persistence_notifier);
        ...
        channel_state
            .pending_msg_events
            .push(events::MessageSendEvent::SendOpenChannel {
            ...
            match channel_state.by_id.entry(channel_id.clone()) {
                hash_map::Entry::Occupied(mut chan_entry) => {
                    ...
                    if let Some(monitor_update) = monitor_update {
                        if let Err(e) = self
                            .chain_monitor
                            .update_channel(chan_entry.get().get_funding_txo().unwrap(), monitor_update)
                        {
                    ...
                    channel_state
                        .pending_msg_events
                        .push(events::MessageSendEvent::SendShutdown {
                            node_id: counterparty_node_id,
                            msg: shutdown_msg,
                        });
                    ...
                    if chan_entry.get().is_shutdown() {
                        ...
                        if let Ok(channel_update) = self.get_channel_update_for_broadcast(&channel) {
                            channel_state
                                .pending_msg_events
                                .push(events::MessageSendEvent::BroadcastChannelUpdate { msg: channel_update });
                        }
    ...
    fn decode_update_add_htlc_onion(
        ...
        macro_rules! return_malformed_err {
            ($msg: expr, $err_code: expr) => {{
                ...
                return (
                    PendingHTLCStatus::Fail(HTLCFailureMsg::Malformed(msgs::UpdateFailMalformedHTLC {
                ...
                if let Some((err, code, chan_update)) = loop {
                    ...
                    let (chan_update_opt, forwardee_cltv_expiry_delta) = if let Some(forwarding_id) = forwarding_id_opt
                    {
                        ...
                        let fee = amt_to_forward
                            ...
                            .and_then(|prop_fee| {
                                (prop_fee / 1000000).checked_add(chan.get_outbound_forwarding_fee_base_msat() as u64)
                            });
                        if fee.is_none()
                    ...
                    if (msg.cltv_expiry as u64) < (*outgoing_cltv_value) as u64 + forwardee_cltv_expiry_delta as u64 {
                    ...
                    if let Some(chan_update) = chan_update {
                        ...
                        (chan_update.serialized_length() as u16)
                            .write(&mut res)
    ...
    fn get_channel_update_for_onion(
        ...
        let sig = self
            .secp_ctx
            .sign(&hash_to_message!(&msg_hash[..]), &self.our_network_key);
    ...
    pub(crate) fn send_payment_along_path(
        ...
        let onion_keys = onion_utils::construct_onion_keys(&self.secp_ctx, &path, &session_priv).map_err(|_| {
            APIError::RouteError {
                err: "Pubkey along hop was maliciously selected",
            }
        })?;
        let (onion_payloads, htlc_msat, htlc_cltv) =
            onion_utils::build_onion_payloads(path, total_value, payment_secret, cur_height, keysend_preimage)?;
        ...
        let err: Result<(), _> = loop {
            ...
            if let hash_map::Entry::Occupied(mut chan) = channel_state.by_id.entry(id) {
                match {
                    ...
                    if !chan.get().is_live() {
                        return Err(APIError::ChannelUnavailable {
                            err: "Peer for first hop currently disconnected/pending monitor update!".to_owned(),
                        });
                    }
                    break_chan_entry!(
                        self,
                    ...
                    Some((update_add, commitment_signed, monitor_update)) => {
                        ...
                        log_debug!(
                            self.logger,
                            "Sending payment along path resulted in a commitment_signed for channel {}",
            ...
            if let Some(payment) = outbounds.get(&payment_id) {
                match payment {
                    ...
                    PendingOutboundPayment::Legacy { .. } => {
                        return Err(PaymentSendFailure::ParameterError(APIError::APIMisuseError {
                            err: "Unable to retry payments that were initially sent on LDK versions prior to 0.0.102"
                                .to_string(),
                        }))
                    },
                ...
                if payment.get().remaining_parts() == 0 {
                    self.pending_events.lock().unwrap().push(events::Event::PaymentFailed {
                        ...
                        payment_hash: payment
                            .get()
                            .payment_hash()
                            .expect("PendingOutboundPayments::RetriesExceeded always has a payment hash set"),
                    });
                    payment.remove();
                }
            }
    ...
    >(
        ...
        channel_state
            .pending_msg_events
            .push(events::MessageSendEvent::SendFundingCreated {
    ...
    pub fn broadcast_node_announcement(&self, rgb: [u8; 3], alias: [u8; 32], mut addresses: Vec<NetAddress>) {
        ...
        if announced_chans {
            channel_state
                .pending_msg_events
                .push(events::MessageSendEvent::BroadcastNodeAnnouncement {
                                ...
                                match forward_info {
                                    HTLCForwardInfo::AddHTLC {
                                        ...
                                        if let PendingHTLCRouting::Forward { onion_packet, .. } = routing {
                                            let phantom_secret_res =
                                                self.keys_manager.get_node_secret(Recipient::PhantomNode);
                                                ...
                                                )
                                            {
                                                ...
                                                let next_hop = match onion_utils::decode_next_hop(
                                                    ...
                                                    Ok(res) => res,
                                                    Err(onion_utils::OnionDecodeErr::Malformed {
                                                        err_msg,
                                                        err_code,
                                                    }) => {
                                                        let sha256_of_onion =
                                                            Sha256::hash(&onion_packet.hop_data).into_inner();
                                ...
                                HTLCForwardInfo::AddHTLC {
                                    ...
                                    match chan.get_mut().send_htlc(
                                        ...
                                        Err(e) => {
                                            ...
                                            let (failure_code, data) = self.get_htlc_temp_fail_err_and_data(
                                                0x1000 | 7,
                                                short_chan_id,
                                                chan.get(),
                                            );
                        ...
                        match forward_info {
                            HTLCForwardInfo::AddHTLC {
                                ...
                                match payment_secrets.entry(payment_hash) {
                                    hash_map::Entry::Vacant(_) => match claimable_htlc.onion_payload {
                                    ...
                                    hash_map::Entry::Occupied(inbound_payment) => {
                                        ...
                                        if inbound_payment.get().payment_secret != payment_data.payment_secret {
                                            ...
                                            && payment_data.total_msat < inbound_payment.get().min_value_msat.unwrap()
                                        {
    ...
    fn update_channel_fee(
        &self,
        short_to_id: &mut HashMap<u64, [u8; 32]>,
        pending_msg_events: &mut Vec<events::MessageSendEvent>,
        chan_id: &[u8; 32],
        chan: &mut Channel<Signer>,
        new_feerate: u32,
        ...
        PersistenceNotifierGuard::optionally_notify(&self.total_consistency_lock, &self.persistence_notifier, || {
            ...
            let mut handle_errors = Vec::new();
            {
                ...
                channel_state.by_id.retain(|chan_id, chan| {
                    let (retain_channel, chan_needs_persist, err) =
                        self.update_channel_fee(short_to_id, pending_msg_events, chan_id, chan, new_feerate);
        ...
        if let Some(mut sources) = removed_source {
            for htlc in sources.drain(..) {
                ...
                htlc_msat_height_data
                    .extend_from_slice(&byte_utils::be32_to_array(self.best_block.read().unwrap().height()));
                self.fail_htlc_backwards_internal(
                        ...
                        if payment.get_mut().remove(&session_priv_bytes, Some(&path)) && !payment.get().is_fulfilled() {
                            ...
                            if payment.get().abandoned() && payment.get().remaining_parts() == 0 {
                                pending_events.push(events::Event::PaymentFailed {
                                    payment_id,
                                    payment_hash: payment.get().payment_hash().expect(
                                        "PendingOutboundPayments::RetriesExceeded always has a payment hash set",
                                    ),
                                });
                                payment.remove();
                            }
    ...
    fn fail_htlc_backwards_internal(
        &self,
        mut channel_state_lock: MutexGuard<ChannelHolder<Signer>>,
        source: HTLCSource,
        payment_hash: &PaymentHash,
                ...
                let err_packet = match onion_error {
                    HTLCFailReason::Reason { failure_code, data } => {
                        ...
                        if let Some(phantom_ss) = phantom_shared_secret {
                            let phantom_packet =
                                onion_utils::build_failure_packet(&phantom_ss, failure_code, &data[..]).encode();
    ...
    pub fn claim_funds(&self, payment_preimage: PaymentPreimage) -> bool {
        ...
        if let Some(mut sources) = removed_source {
            ...
            for htlc in sources.iter() {
                if let None = channel_state
                    .as_ref()
                    .unwrap()
                    .short_to_id
                    .get(&htlc.prev_hop.short_channel_id)
                {
                    valid_mpp = false;
                    break;
                }
    ...
    fn claim_funds_from_hop(
        &self,
        channel_state_lock: &mut MutexGuard<ChannelHolder<Signer>>,
        prev_hop: HTLCPreviousHopData,
        payment_preimage: PaymentPreimage,
    ) -> ClaimFundsFromHop {
        ...
        if let hash_map::Entry::Occupied(mut chan) = channel_state.by_id.entry(chan_id) {
            match chan
                .get_mut()
                .get_update_fulfill_htlc_and_commit(prev_hop.htlc_id, payment_preimage, &self.logger)
            {
    ...
    fn claim_funds_internal(
        ...
        match source {
            ...
            HTLCSource::PreviousHopData(hop_data) => {
                ...
                let claimed_htlc = if let ClaimFundsFromHop::DuplicateClaim = res {
                    false
                } else {
                    true
                };
    ...
    pub fn accept_inbound_channel(
        ...
        match channel_state.by_id.entry(temporary_channel_id.clone()) {
            hash_map::Entry::Occupied(mut channel) => {
                ...
                channel_state
                    .pending_msg_events
                    .push(events::MessageSendEvent::SendAcceptChannel {
                        node_id: channel.get().get_counterparty_node_id(),
    ...
    fn internal_funding_created(
        ...
        match channel_state.by_id.entry(funding_msg.channel_id) {
            ...
            hash_map::Entry::Vacant(e) => {
                channel_state
                    .pending_msg_events
                    .push(events::MessageSendEvent::SendFundingSigned {
                        node_id: counterparty_node_id.clone(),
                        msg: funding_msg,
                    });
                e.insert(chan);
            ...
            hash_map::Entry::Occupied(mut chan) => {
                ...
                if let Some(announcement_sigs) = announcement_sigs_opt {
                    ...
                    if let Ok(msg) = self.get_channel_update_for_unicast(chan.get()) {
                        channel_state
                            .pending_msg_events
                            .push(events::MessageSendEvent::SendChannelUpdate {
                                node_id: counterparty_node_id.clone(),
                                msg,
                            });
                    }
            ...
            match channel_state.by_id.entry(msg.channel_id.clone()) {
                hash_map::Entry::Occupied(mut chan_entry) => {
                    ...
                    if !chan_entry.get().received_shutdown() {
                        log_info!(
                            self.logger,
                            "Received a shutdown message from our counterparty for channel {}{}.",
                            log_bytes!(msg.channel_id),
                            if chan_entry.get().sent_shutdown() {
                    ...
                    let (shutdown, monitor_update, htlcs) = try_chan_entry!(
                        self,
                        chan_entry.get_mut().shutdown(&self.keys_manager, &their_features, &msg),
                        channel_state,
                        chan_entry
                    );
                    dropped_htlcs = htlcs;
            ...
            match channel_state.by_id.entry(msg.channel_id.clone()) {
                hash_map::Entry::Occupied(mut chan_entry) => {
                    ...
                    if let Some(msg) = closing_signed {
                        channel_state
                            .pending_msg_events
                            .push(events::MessageSendEvent::SendClosingSigned {
                                node_id: counterparty_node_id.clone(),
                                msg,
                            });
                    }
        ...
        match channel_state.by_id.entry(msg.channel_id) {
            hash_map::Entry::Occupied(mut chan) => {
                ...
                let create_pending_htlc_status =
                    |chan: &Channel<Signer>, pending_forward_info: PendingHTLCStatus, error_code: u16| {
                        ...
                        match pending_forward_info {
                            PendingHTLCStatus::Forward(PendingHTLCInfo {
                                ...
                                let reason = if (error_code & 0x1000) != 0 {
                                    let (real_code, error_data) =
                                        self.get_htlc_inbound_temp_fail_err_and_data(error_code, chan);
            ...
            hash_map::Entry::Occupied(mut chan) => {
                ...
                if (msg.failure_code & 0x8000) == 0 {
                    let chan_err: ChannelError =
                        ChannelError::Close("Got update_fail_malformed_htlc with BADONION not set".to_owned());
    ...
    fn internal_revoke_and_ack(
        ...
        let res = loop {
            ...
            match channel_state.by_id.entry(msg.channel_id) {
                hash_map::Entry::Occupied(mut chan) => {
                    ...
                    if let Err(e) = self
                        .chain_monitor
                        .update_channel(chan.get().get_funding_txo().unwrap(), raa_updates.monitor_update)
                    {
                        if was_frozen_for_monitor {
                            ...
                            break Err(MsgHandleErrInternal::ignore_no_close(
                                "Previous monitor update failure prevented responses to RAA".to_owned(),
                            ));
                        } else {
        ...
        match res {
            Ok((pending_forwards, mut pending_failures, finalized_claim_htlcs, short_channel_id, channel_outpoint)) => {
                for failure in pending_failures.drain(..) {
                    self.fail_htlc_backwards_internal(
                        self.channel_state.lock().unwrap(),
                        failure.0,
            ...
            hash_map::Entry::Occupied(mut chan) => {
                ...
                if !chan.get().is_usable() {
                    return Err(MsgHandleErrInternal::from_no_close(LightningError {
                        err: "Got an announcement_signatures before we were ready for it".to_owned(),
                        action: msgs::ErrorAction::IgnoreError,
                    }));
                }
    ...
    fn internal_channel_update(
        ...
        match channel_state.by_id.entry(chan_id) {
            hash_map::Entry::Occupied(mut chan) => {
                ...
                let were_node_one =
                    self.get_our_node_id().serialize()[..] < chan.get().get_counterparty_node_id().serialize()[..];
    ...
    fn set_payment_hash_secret_map(
        ...
        match payment_secrets.entry(payment_hash) {
            ...
            hash_map::Entry::Occupied(_) => {
                return Err(APIError::APIMisuseError {
                    err: "Duplicate payment hash".to_owned(),
                })
            },
        }
        Ok(payment_secret)
    ...
    L::Target: Logger,
{
    ...
    fn best_block_updated(&self, header: &BlockHeader, height: u32) {
        ...
        *self.best_block.write().unwrap() = BestBlock::new(block_hash, height);
        ...
        let mut payment_secrets = self.pending_inbound_payments.lock().unwrap();
        payment_secrets.retain(|_, inbound_payment| inbound_payment.expiry_time > header.time as u64);
    ...
    L::Target: Logger,
{
    ...
    pub fn new(
        ...
        mut channel_monitors: Vec<&'a mut ChannelMonitor<Signer>>,
    ) -> Self {
        Self {
            keys_manager,
            fee_estimator,
    ...
    L::Target: Logger,
{
    fn read<R: io::Read>(
        reader: &mut R,
        mut args: ChannelManagerReadArgs<'a, Signer, M, T, K, F, L>,
    ) -> Result<Self, DecodeError> {
        ...
        let mut pending_background_events_read: Vec<BackgroundEvent> = Vec::with_capacity(cmp::min(
        ...
        let mut pending_inbound_payments: HashMap<PaymentHash, PendingInboundPayment> = HashMap::with_capacity(
            cmp::min(pending_inbound_payment_count as usize, MAX_ALLOC_SIZE / (3 * 32)),
        );
        ...
        let mut pending_outbound_payments_compat: HashMap<PaymentId, PendingOutboundPayment> = HashMap::with_capacity(
            cmp::min(pending_outbound_payments_count_compat as usize, MAX_ALLOC_SIZE / 32),
        );
...
mod tests {
    ...
    fn test_notify_limits() {
        ...
        nodes[2]
            .node
            .handle_channel_update(&nodes[1].node.get_our_node_id(), &chan.0);
        nodes[2]
            .node
            .handle_channel_update(&nodes[1].node.get_our_node_id(), &chan.1);
        assert!(!nodes[2].node.await_persistable_update_timeout(Duration::from_millis(1)));
        ...
        nodes[0]
            .node
            .handle_channel_update(&nodes[2].node.get_our_node_id(), &chan.0);
        nodes[0]
            .node
            .handle_channel_update(&nodes[2].node.get_our_node_id(), &chan.1);
        nodes[1]
            .node
            .handle_channel_update(&nodes[2].node.get_our_node_id(), &chan.0);
        nodes[1]
        ...
        let as_update = if as_node_one == (chan.0.contents.flags & 1 == 0/* chan.0 is from node one */) {
            &chan.0
        } else {
            &chan.1
        };
        let bs_update = if as_node_one == (chan.0.contents.flags & 1 == 0/* chan.0 is from node one */) {
            &chan.1
        } else {
            &chan.0
        };
        ...
        nodes[0]
            .node
            .handle_channel_update(&nodes[1].node.get_our_node_id(), &as_update);
        nodes[1]
            .node
            .handle_channel_update(&nodes[0].node.get_our_node_id(), &bs_update);
        assert!(!nodes[0].node.await_persistable_update_timeout(Duration::from_millis(1)));
        assert!(!nodes[1].node.await_persistable_update_timeout(Duration::from_millis(1)));
        assert_eq!(nodes[0].node.list_channels()[0], node_a_chan_info);
        assert_eq!(nodes[1].node.list_channels()[0], node_b_chan_info);
        ...
        nodes[0]
            .node
            .handle_channel_update(&nodes[1].node.get_our_node_id(), &bs_update);
        nodes[1]
            .node
            .handle_channel_update(&nodes[0].node.get_our_node_id(), &as_update);
        assert!(nodes[0].node.await_persistable_update_timeout(Duration::from_millis(1)));
        assert!(nodes[1].node.await_persistable_update_timeout(Duration::from_millis(1)));
    ...
    fn test_keysend_dup_hash_partial_mpp() {
        ...
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_first_updates.commitment_signed);
        check_added_monitors!(nodes[0], 1);
        let (as_first_raa, as_first_cs) = get_revoke_commit_msgs!(nodes[0], nodes[1].node.get_our_node_id());
        nodes[1]
            .node
            .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_first_raa);
        check_added_monitors!(nodes[1], 1);
        let bs_second_updates = get_htlc_update_msgs!(nodes[1], nodes[0].node.get_our_node_id());
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_first_cs);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_second_updates.commitment_signed);
        check_added_monitors!(nodes[0], 1);
        ...
        nodes[0]
            .node
            .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_first_raa);
        let as_second_updates = get_htlc_update_msgs!(nodes[0], nodes[1].node.get_our_node_id());
        check_added_monitors!(nodes[0], 1);
        nodes[1]
            .node
            .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_second_raa);
        check_added_monitors!(nodes[1], 1);
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_second_updates.commitment_signed);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[0]
            .node
            .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_third_raa);
        check_added_monitors!(nodes[0], 1);
    ...
    fn test_keysend_dup_payment_hash() {
        ...
        nodes[0]
            .node
            .send_payment(&route, payment_hash, &Some(payment_secret))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
        let mut events = nodes[0].node.get_and_clear_pending_msg_events();
        assert_eq!(events.len(), 1);
    ...
    fn test_keysend_hash_mismatch() {
        ...
        let _ = nodes[0]
            .node
            .send_payment_internal(&route, mismatch_payment_hash, &None, Some(test_preimage), None, None)
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    ...
    fn test_multi_hop_missing_secret() {
        ...
        let chan_1_id = create_announced_chan_between_nodes(&nodes, 0, 1, InitFeatures::known(), InitFeatures::known())
            ...
            .contents
            .short_channel_id;
        let chan_2_id = create_announced_chan_between_nodes(&nodes, 0, 2, InitFeatures::known(), InitFeatures::known())
            .0
            .contents
            .short_channel_id;
...
pub mod bench {
    ...
    pub fn bench_two_sends<P: Persist<InMemorySigner>>(bench: &mut Bencher, persister_a: P, persister_b: P) {
        ...
        node_a
            .create_channel(node_b.get_our_node_id(), 8_000_000, 100_000_000, 42, None)
            .unwrap();
        ...
        macro_rules! send_payment {
            ($node_a: expr, $node_b: expr) => {
                ...
                let payment_params =
                    PaymentParameters::from_node_id($node_b.get_our_node_id()).with_features(InvoiceFeatures::known());
                let scorer = test_utils::TestScorer::with_penalty(0);
                let seed = [3u8; 32];
                ...
                match $node_b.get_and_clear_pending_msg_events().pop().unwrap() {
                    MessageSendEvent::UpdateHTLCs { node_id, updates } => {
                        ...
                        $node_a
                            .handle_update_fulfill_htlc(&$node_b.get_our_node_id(), &updates.update_fulfill_htlcs[0]);
